/**
 * backtest-step-trail-sizes-3y.ts
 *
 * 3-year LIVE engine backtest sweeping STEP SIZE for 15m trailing (NO fixed TP cap).
 * 6 modes total: E0 baseline + S50/S60/S70/S80/S90.
 *   - S{N}: 10 levels at k×N% of TP distance for k = 1..10.
 *
 * For HTF (1h/4h/1d/1w): keep fixed SL + TP (no trailing).
 *
 * Reuses .cache/binance-{tf}-3y.json.
 *
 * Output:
 *   - assets/backtest_step_trail_sizes_3y.json
 *   - assets/backtest_step_trail_sizes_3y_report.html
 *
 * Usage:
 *   npx tsx tools/backtest-step-trail-sizes-3y.ts
 */
...
// Step trailing spec (15m only) — sweep size S, 10 levels at k×S for k=1..10
const STEP_SIZES = [0.5, 0.6, 0.7, 0.8, 0.9]; // 50%, 60%, 70%, 80%, 90%
function buildSteps(size: number): number[] {
  return Array.from({ length: 10 }, (_, i) => +(size * (i + 1)).toFixed(6));
}
...
<body>
<h1>📈 STEP TRAIL SIZE SWEEP · NO TP CAP · 3-YEAR BACKTEST · BTC/USDT</h1>
...
<b>Step trail (15m TF only, NO fixed TP cap):</b> S{N} = 10 levels at k×N% of TP distance for k = 1..10. Sizes tested: ${STEP_SIZES.map(s => `${(s * 100).toFixed(0)}%`).join(", ")}.<br>
HTF (1h/4h/1d/1w) keep fixed TP/SL.
</div>

<div class="card">
  <h2>📊 EQUITY OVERLAY · ${modes.length} modes</h2>
  ${overlay}
</div>
...
<div class="card">
  <h2>📊 HIT-TYPE BREAKDOWN (${modes.length} modes)</h2>
  <div class="three-col">
...
// ─── Main ───────────────────────────────────────────────────────────────────
(async () => {
  console.log(`\n=== STEP TRAIL SIZE SWEEP · NO TP CAP · BACKTEST 3Y · BTC/USDT ===`);
  for (const s of STEP_SIZES) {
    const steps = buildSteps(s);
    console.log(`  S${(s * 100).toFixed(0)}: ${steps.map(v => (v * 100).toFixed(0) + "%").join(", ")}`);
  }
  console.log("");
  ...
  // Run baseline + 5 step-size variants
  const runDefs: { name: string; desc: string; mode: StepMode; steps: number[] }[] = [
    { name: "E0", desc: "Baseline · no trail, fixed TP/SL all", mode: "off", steps: buildSteps(0.5) },
  ];
  for (const s of STEP_SIZES) {
    const pct = (s * 100).toFixed(0);
    runDefs.push({
      name: `S${pct}`,
      desc: `Step trail 15m, NO fixed TP, levels ${buildSteps(s).map(v => (v * 100).toFixed(0) + "%").join("/")}`,
      mode: "noTp",
      steps: buildSteps(s),
    });
  }
  ...
  const out = {
    ...
    config: {
      ...
      stepTrail: {
        sizes: STEP_SIZES,
        stepsBySize: Object.fromEntries(STEP_SIZES.map((s) => [`S${(s * 100).toFixed(0)}`, buildSteps(s)])),
  ...
  const jsonPath = join(assetsDir, "backtest_step_trail_sizes_3y.json");
  writeFileSync(jsonPath, JSON.stringify(out, null, 2));
  console.log(`\n💾 JSON: ${jsonPath}`);

  const htmlPath = join(assetsDir, "backtest_step_trail_sizes_3y_report.html");
  ...
  for (const m of results) {
    console.log(`${m.modeName.padEnd(6)}: NET ${m.metrics.netPctLev}%, DD -${m.metrics.maxDD}%, ${m.metrics.trades} trades, WR ${m.metrics.winRate}%, PF ${m.metrics.profitFactor === 999 ? "∞" : m.metrics.profitFactor}, Sharpe ${m.metrics.sharpeLike}, avgHold ${m.metrics.avgHoldBars}`);
  }
  console.log(`\n=== Δ vs E0 ===`);
  for (const d of out.deltasVsE0) {
    if (d.modeName === "E0") continue;
    console.log(`${d.modeName.padEnd(6)}: ΔNET ${d.deltaNetPctLev >= 0 ? "+" : ""}${d.deltaNetPctLev}% · ΔDD ${d.deltaMaxDD >= 0 ? "+" : ""}${d.deltaMaxDD}% · ΔWR ${d.deltaWinRate >= 0 ? "+" : ""}${d.deltaWinRate}pp · ΔPF ${d.deltaProfitFactor >= 0 ? "+" : ""}${d.deltaProfitFactor} · ΔSharpe ${d.deltaSharpe >= 0 ? "+" : ""}${d.deltaSharpe} · ΔTrades ${d.deltaTrades >= 0 ? "+" : ""}${d.deltaTrades}`);
  }

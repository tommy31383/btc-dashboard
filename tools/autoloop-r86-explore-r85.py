#!/usr/bin/env python3
"""autoloop-r86-explore-r85.py — Round 86: Explore with R85 config
R85 FINAL: SOL ABM=1.5 → Sh+2.43 DD4.6% (3/3 robust)

R86:
  A: Full verify R85 vs R50 (definitive)
  B: SOL ADX thresh sweep with ABM=1.5 (might need re-tune)
  C: SOL SL_TRANS sweep with ABM=1.5 (ST=24 optimal?)
  D: BTC D_LB=18 sensitivity re-confirm with R85 baseline
  E: NEW direction: SOL volume filter (vol_mult) with ABM=1.5
"""
...
print("Loading R85 config...")
moB,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16)
moS,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=1.5)
turB14=run_turtle()
...
def pr(label, parts, weights=None, cal=None, extra=""):
    ...
    ok=sh>=2.42; mark="✅✅✅✅✅" if ok else ("✅✅✅✅" if sh>=2.38 else ("✅✅✅" if sh>=2.30 else ("✅✅" if sh>=2.25 else ("✅" if sh>=2.10 else ("⚠️" if sh>=2.00 else "")))))
    star="★★★★★" if sh>=2.50 else ("★★★★" if sh>=2.47 else ("★★★" if sh>=2.44 else ("★★" if sh>=2.43 else ("★" if delta>0.05 else ""))))
    print(f"  {mark} {label:<54} Sh{sh:>+5.2f} DD{md:>4.1f}% flat{fl:>3} TE{te:>+5.2f} d{delta:>+.3f} {star} {extra}")
    return sh,md

sh_base,_=pr("R85 FINAL (SOL ABM=1.5)",[sB,sS,sTB14])
print(); print("="*100); print("=== R86: Explore with R85 config ===\n")

# A: Full verify R85 vs R50
print("━"*100); print("A: R85 vs R50 — full holdout")
...
for yr_exc in [2023,2024,2025]:
    ...
    sh_r85,md_r85,_,_,_=bk([sB_ho,sS_ho,sTB14_ho],W,cal=cal_ho)
    delta=sh_r85-sh_can
    print(f"  Holdout {yr_exc}: R50 Sh{sh_can:+.2f} DD{md_can:.1f}% → R85 Sh{sh_r85:+.2f} DD{md_r85:.1f}% delta{delta:>+.3f} {'✓' if delta>0 else '✗'}")

# B: SOL ADX thresh with ABM=1.5
print(f"\n{'━'*100}"); print("B: SOL ADX thresh sweep with ABM=1.5")
for adxt in [12,13,14,15,16,17,18]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",adxt,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=1.5)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL_ADX={adxt}",[sB,sS_,sTB14],extra=f"n={nt}")

# C: SOL SL_TRANS with ABM=1.5
print(f"\n{'━'*100}"); print("C: SOL SL_TRANS with ABM=1.5 (current=24)")
for st_ in [12,16,20,24,32,48]:
    ...
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL ST={st_}",[sB,sS_,sTB14],extra=f"n={nt}")

# D: BTC D_LB re-confirm with R85 baseline
print(f"\n{'━'*100}"); print("D: BTC D_LB re-confirm with R85 baseline")
for dlb in [15,16,17,18,19,20,22]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=dlb,vol_ma=16)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC D_LB={dlb}",[sB_,sS,sTB14],extra=f"n={nt}")

# E: SOL vol_mult with ABM=1.5
print(f"\n{'━'*100}"); print("E: SOL vol_mult sweep with ABM=1.5 (current=1.4)")
for vm in [1.1,1.2,1.3,1.4,1.5,1.6]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=1.5,vol_mult=vm)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL vm={vm}",[sB,sS_,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R86 SUMMARY — Ceiling Sh{sh_base:+.3f}")
print(f"  R85 FINAL CONFIG: Sh+2.43 DD4.6%")

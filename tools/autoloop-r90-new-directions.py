#!/usr/bin/env python3
"""autoloop-r90-new-directions.py — Round 90: Genuinely new directions
R87-R89: ALL PARAMS CONFIRMED. Ceiling Sh+2.48 DD4.6%.

R90 tries GENUINELY NEW approaches not yet explored:
  A: REGIME PERSISTENCE param — how many days regime must persist before valid
     (default is built into H.regime_with_persistence — test different N)
  B: BTC DONCHIAN CD (S14 cooldown) — currently S14 CD=36, test lower
  C: S14-only (no S13) vs current 3-signal — quality vs quantity
  D: BTC AUG-SKIP month sensitivity — is September also worth skipping?
  E: SOL AUG-SKIP — does SOL benefit from skipping August too?
  F: ADX lookback (Wilder ADX period) interaction with D_LB=18
"""
...
def run_h01(cache, adx_thresh=18, sl_init=3.0, sl_trail=3.5, sl_trans=16,
             ...
             skip_months=None, donchian_lb=18, s14_cd=36, use_s13=True):
    H.CACHE=cache
    ...
    sigs={"S12":(s12,False,36),"S14":(s14,True,s14_cd)}
    if use_s13: sigs["S13"]=(s13,True,1)
...
print("Loading R87 baselines...")
moB,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16)
moS,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=20,donchian_lb=20,vol_ma=16,atr_break_mult=1.5)
turB14=run_turtle()
...
def pr(label, parts, weights=None, cal=None, extra=""):
    ...
    ok=sh>=2.47; mark="✅✅✅✅✅" if ok else ("✅✅✅✅" if sh>=2.42 else ("✅✅✅" if sh>=2.35 else ("✅✅" if sh>=2.25 else ("✅" if sh>=2.10 else ("⚠️" if sh>=2.00 else "")))))
    star="★★★★★" if sh>=2.55 else ("★★★★" if sh>=2.52 else ("★★★" if sh>=2.50 else ("★★" if sh>=2.48 else ("★" if delta>0.05 else ""))))
    print(f"  {mark} {label:<54} Sh{sh:>+5.2f} DD{md:>4.1f}% flat{fl:>3} TE{te:>+5.2f} d{delta:>+.3f} {star} {extra}")
    return sh,md

sh_base,_=pr("R87 BASELINE",[sB,sS,sTB14])
print(); print("="*100); print("=== R90: New directions ===\n")

# A: S14 cooldown sweep
print("━"*100); print("A: S14 CD sweep (current=36) — allow S14 to re-fire sooner?")
for cd14 in [12,16,20,24,28,36,48,60]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16,s14_cd=cd14)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  S14_CD={cd14}",[sB_,sS,sTB14],extra=f"n={nt}")

# B: S14 only (no S13) — quality filter
print(f"\n{'━'*100}"); print("B: S14-only BTC (no S13) vs current 3-signal — higher quality?")
moB_s14,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16,use_s13=False)
sB_s14=[moB_s14.get(m,0.0) for m in cal3]
pr(f"  S14+S12 only (no S13)",[sB_s14,sS,sTB14],extra=f"n={nt}")
pr(f"  Full 3-signal (baseline)",[sB,sS,sTB14])

# C: BTC Aug+Sep skip test (Sep also borderline bad)
print(f"\n{'━'*100}"); print("C: BTC Aug+Sep skip — add September")
moB_as,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8,9},donchian_lb=18,vol_ma=16)
sB_as=[moB_as.get(m,0.0) for m in cal3]
pr(f"  BTC skip Aug+Sep",[sB_as,sS,sTB14],extra=f"n={nt}")
pr(f"  BTC skip Aug only (baseline)",[sB,sS,sTB14])
# holdout
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sBaug_ho=[moB.get(m,0.0) for m in cal_ho]; sBas_ho=[moB_as.get(m,0.0) for m in cal_ho]
    sS_ho=[moS.get(m,0.0) for m in cal_ho]; sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_a,_,_,_,_=bk([sBaug_ho,sS_ho,sTB_ho],cal=cal_ho)
    sh_as,_,_,_,_=bk([sBas_ho,sS_ho,sTB_ho],cal=cal_ho)
    robust="✓" if sh_as>sh_a else "✗"
    print(f"  Holdout {yr_exc}: Aug Sh{sh_a:+.2f} → Aug+Sep Sh{sh_as:+.2f} delta{sh_as-sh_a:>+.3f} {robust}")

# D: SOL Aug skip
print(f"\n{'━'*100}"); print("D: SOL Aug skip (SOL has no August trades in 3y, verify)")
aug_sol_trades=[m for m in cal3 if m[5:7]=="08" and abs(moS.get(m,0.0))>1e-9]
print(f"  SOL August trades existing: {aug_sol_trades}")
moS_aug,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=20,donchian_lb=20,vol_ma=16,atr_break_mult=1.5,skip_months={8})
sS_aug=[moS_aug.get(m,0.0) for m in cal3]
pr(f"  SOL skip Aug too",[sB,sS_aug,sTB14],extra=f"n={nt}")

# E: Different months to skip for BTC
print(f"\n{'━'*100}"); print("E: BTC other month skips (7y data confirmed bad: Mo1=-0.9%, Mo8=-1.3%, Mo9=-0.7%)")
for skip_mo in [{8},{1},{9},{8,1},{1,8,9}]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months=skip_mo,donchian_lb=18,vol_ma=16)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  skip={skip_mo}",[sB_,sS,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R90 SUMMARY — Ceiling Sh{sh_base:+.3f}")

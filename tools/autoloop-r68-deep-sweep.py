#!/usr/bin/env python3
"""autoloop-r68-deep-sweep.py — Round 68: Deep sweep new directions
R67 ceiling: BTC(SL2.8/3.3) + SOL(SL2.9/3.4/ST24) + Turtle(T1.2) → Sh+2.16 DD4.9%
New directions to explore:
  A: Turtle DX param (exit channel) — current=15, try 8-25
  B: 7y verify — R67 final config vs canonical on full 7y BTC data
  C: ADX threshold 17 (between 18 optimal and 15 borderline) — BTC+SOL
  D: BTC ADX threshold 17 — does SL2.8/3.3 change the ADX optimal?
  E: Turtle DE param (entry channel) — current=20, try 15-30
  F: SOL 7y simulation — if SOL had 7y data, how does method scale?
"""
...
print("Loading R67 final config...")
...
sh_base=pr("R67 FINAL CONFIG",[sB28,sS29,sTB])[0]
print(); print("="*100); print("=== R68: Deep sweep — Turtle DE/DX + ADX thresh ===\n")

# A: Turtle DX (exit channel)
print("━"*100); print("A: Turtle DX sweep (exit Donchian) — current=15")
for DX in [8,10,12,14,15,16,18,20,25]:
    tur_=run_turtle(20,DX,2.0); sTB_=[tur_.get(m,0.0) for m in cal3]
    pr(f"  Turtle DX={DX}",[sB28,sS29,sTB_])

# B: 7y verify of R67 config
print(f"\n{'━'*100}"); print("B: 7y BTC performance — R67 vs canonical")
...
sh_can7=sharpe(sB7_can); sh_new7=sharpe(sB7_new)
md_can7=maxdd(sB7_can); md_new7=maxdd(sB7_new)
...
print(f"  BTC-CANONICAL 7y: Sh{sh_can7:+.2f} DD{md_can7:.1f}% | per-yr: {' '.join(f'{y%100}:{yr_can[y]*100:+.0f}' for y in sorted(yr_can))}")
print(f"  BTC-SL2.8/3.3 7y: Sh{sh_new7:+.2f} DD{md_new7:.1f}% | per-yr: {' '.join(f'{y%100}:{yr_new[y]*100:+.0f}' for y in sorted(yr_new))}")

# C: ADX threshold BTC+SOL sweep with new SL
print(f"\n{'━'*100}"); print("C: ADX threshold sweep BTC (with SL2.8/3.3)")
for adxt in [15,16,17,18,19,20,22]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",adxt,sl_init=2.8,sl_trail=3.3)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC_ADX={adxt}",[sB_,sS29,sTB],extra=f"n={nt}")

# D: SOL ADX threshold with new SL
print(f"\n{'━'*100}"); print("D: SOL ADX threshold sweep with SL2.9/3.4")
for adxt in [12,13,14,15,16,17,18]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",adxt,sl_init=2.9,sl_trail=3.4,sl_trans=24)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL_ADX={adxt}",[sB28,sS_,sTB],extra=f"n={nt}")

# E: Turtle DE (entry channel)
print(f"\n{'━'*100}"); print("E: Turtle DE sweep (entry Donchian) — current=20")
for DE in [10,14,16,18,20,22,24,28,32]:
    tur_=run_turtle(DE,15,2.0); sTB_=[tur_.get(m,0.0) for m in cal3]
    sh_tur_7=sharpe([tur_.get(m,0.0) for m in cal7])
    pr(f"  Turtle DE={DE}",[sB28,sS29,sTB_],extra=f"tur7y_Sh{sh_tur_7:+.2f}")

# F: DE+DX combination
print(f"\n{'━'*100}"); print("F: Turtle DE+DX fine combo (around 20/15)")
for DE,DX in [(18,14),(18,15),(20,14),(20,15),(20,16),(22,15),(22,16),(24,15)]:
    tur_=run_turtle(DE,DX,2.0); sTB_=[tur_.get(m,0.0) for m in cal3]
    pr(f"  Turtle DE={DE} DX={DX}",[sB28,sS29,sTB_])

print(f"\n{'='*100}")
print(f"R68 SUMMARY — ceiling Sh{sh_base:+.3f}")

/**
 * backtest-hedge01-v0438-aggressive.ts — Test 4 "đánh nhiều có lãi" scenarios.
 *
 * Scenarios:
 *   V0 baseline v0.4.37
 *   V1 Hyperactive — cooldown 1h→30m, score ≥9→≥8, MAX_QTY 0.02→0.04
 *   V2 + Setup #17 Scalp (5m RSI extreme + BB break, qty 0.002, TP+0.8%/SL-0.4%, CD 30m)
 *   V3 + Aggressive BULL DCA (RSI<50 BULL regime → LONG every 15m, TP+5% no SL)
 *   V4 V1 + V2 + V3 ALL combined (max fire)
 */
...
// === Setup #18 Aggressive BULL DCA (V3/V4) ===
const SETUP18_QTY = 0.002;
const SETUP18_TP_PCT = 5;
const SETUP18_COOLDOWN_MS = 15 * 60_000;
const SETUP18_TIMESTOP_MS = 7 * 24 * 60 * 60_000;

interface ScenarioConfig {
  name: string;
  // Hyperactive (V1/V4)
  aggCooldownMs: number;        // 1h or 30m
  aggScoreMin: number;          // 9 or 8
  maxQty: number;               // 0.02 or 0.04
  setup11CooldownMs: number;    // 15m or 10m
  setup13CooldownMs: number;    // 4h or 1h
  // Setup #17 Scalp
  enableScalp17: boolean;
  // Setup #18 Aggressive DCA
  enableDCA18: boolean;
}

const SCENARIOS: ScenarioConfig[] = [
  { name: "V0_baseline_v0437", aggCooldownMs: 60*60_000, aggScoreMin: 9, maxQty: 0.02, setup11CooldownMs: 15*60_000, setup13CooldownMs: 4*60*60_000, enableScalp17: false, enableDCA18: false },
  { name: "V1_hyperactive",    aggCooldownMs: 30*60_000, aggScoreMin: 8, maxQty: 0.04, setup11CooldownMs: 10*60_000, setup13CooldownMs: 1*60*60_000, enableScalp17: false, enableDCA18: false },
  { name: "V2_scalp17",        aggCooldownMs: 60*60_000, aggScoreMin: 9, maxQty: 0.02, setup11CooldownMs: 15*60_000, setup13CooldownMs: 4*60*60_000, enableScalp17: true,  enableDCA18: false },
  { name: "V3_DCA18",          aggCooldownMs: 60*60_000, aggScoreMin: 9, maxQty: 0.02, setup11CooldownMs: 15*60_000, setup13CooldownMs: 4*60*60_000, enableScalp17: false, enableDCA18: true },
  { name: "V4_ALL_combined",   aggCooldownMs: 30*60_000, aggScoreMin: 8, maxQty: 0.04, setup11CooldownMs: 10*60_000, setup13CooldownMs: 1*60*60_000, enableScalp17: true,  enableDCA18: true },
];
...
function aggregateQty(score: number, side: "LONG" | "SHORT", minScore: number): number {
  let qty = 0;
  if (score === 11) qty += 0.001 * 3;
  if (score === 11) qty += 0.01;
  if (score >= 10) qty += 0.001;
  if (score >= minScore) qty += 0.001;  // adjustable threshold
...
function runScenario(c5: Candle[], c15: Candle[], c1h: Candle[], c4h: Candle[], c1d: Candle[], c1w: Candle[], funding: FundingPoint[], ind: any, cfg: ScenarioConfig): any {
  ...
  let lastS17LMs = 0, lastS17SMs = 0, lastS18Ms = 0;
  ...
  // BB on 5m for Setup #17
  const ma20_5 = calcSMA(c5.map(b => b.close), 20);
  const sd20_5 = calcStdev(c5.map(b => b.close), 20, ma20_5);
  const rsi5 = calcRSI(c5.map(b => b.close), 14);
  ...
  for (let i = 0; i < c5.length; i++) {
    ...
    // Close simple trades (S15/S16/S17/S18 — all use SimpleTrade structure)
    ...
    const mom5_15 = idx15c >= 5 ? (last.close - c15[idx15c - 5].close) / c15[idx15c - 5].close * 100 : 0;
    const mom10_15 = idx15c >= 10 ? (last.close - c15[idx15c - 10].close) / c15[idx15c - 10].close * 100 : 0;
    const mom20_15 = idx15c >= 20 ? (last.close - c15[idx15c - 20].close) / c15[idx15c - 20].close * 100 : 0;
    ...
    if (distMA50 <= -3) lS++; if (mom5_15 < 0 && mom10_15 < 0 && mom20_15 < 0) lS++;
    ...
    if (distMA50 >= 3) sS++; if (mom5_15 > 0 && mom10_15 > 0 && mom20_15 > 0) sS++;

    let c2Fire = false;
    if (idx5c >= 22) {
      const last5 = c5[idx5c]; const r5v = ind.rsi5[idx5c];
      if (r5v !== null && r5v >= 70) {
    ...
    const longCD = ts - lastEntryLongMs < cfg.aggCooldownMs;
    const shortCD = ts - lastEntryShortMs < cfg.aggCooldownMs;
    const s11CD = ts - lastSetup11Ms < cfg.setup11CooldownMs;
    const longScale = regime === "BEAR" ? 0.5 : 1.0;
    const allowMomLong = regime !== "BEAR";
    const allowShortSide = regime !== "BULL";

    // Aggregate LONG with adjustable score min
    if (!longCD) {
      let qty = 0;
      let setups: string[] = [];
      if (lS >= cfg.aggScoreMin) {
        qty += aggregateQty(lS, "LONG", cfg.aggScoreMin); setups.push(`agg(${lS})`);
        if (idx5c >= 200) {
      ...
      if (qty > 0 && longNet.qty + qty <= cfg.maxQty) {
    ...
    if (!s11CD && c2Fire && f4Pass && allowMomLong) {
      ...
      if (sq >= MIN_BINANCE_QTY && longNet.qty + sq <= cfg.maxQty) {
    ...
    const lastS12LRef = { get v() { return lastS12L; }, set v(x: number) { lastS12L = x; } };
    const lastS12SRef = { get v() { return lastS12S; }, set v(x: number) { lastS12S = x; } };
    const lastS13LRef = { get v() { return lastS13L; }, set v(x: number) { lastS13L = x; } };
    const lastS13SRef = { get v() { return lastS13S; }, set v(x: number) { lastS13S = x; } };
    const lastS14LRef = { get v() { return lastS14L; }, set v(x: number) { lastS14L = x; } };
    const lastS14SRef = { get v() { return lastS14S; }, set v(x: number) { lastS14S = x; } };

    if (ema12 === "LONG") trendEnter("S12", "LONG", SETUP12_QTY, lastS12LRef as any, 12 * 60 * 60_000);
    else if (ema12 === "SHORT") trendEnter("S12", "SHORT", SETUP12_QTY, lastS12SRef as any, 12 * 60 * 60_000);
    if (atr13 === "LONG") trendEnter("S13", "LONG", SETUP13_QTY, lastS13LRef as any, cfg.setup13CooldownMs);
    else if (atr13 === "SHORT") trendEnter("S13", "SHORT", SETUP13_QTY, lastS13SRef as any, cfg.setup13CooldownMs);
    if (don14 === "LONG") trendEnter("S14", "LONG", SETUP14_QTY, lastS14LRef as any, 12 * 60 * 60_000);
    else if (don14 === "SHORT") trendEnter("S14", "SHORT", SETUP14_QTY, lastS14SRef as any, 12 * 60 * 60_000);

    // SHORT aggregate
    if (sS >= cfg.aggScoreMin && !shortCD && allowShortSide) {
      let weeklyDown = false;
      if (idx1w >= 2) weeklyDown = c1w[idx1w - 1].close < c1w[idx1w - 2].close;
      if (weeklyDown) {
        let qty = aggregateQty(sS, "SHORT", cfg.aggScoreMin);
        qty *= hwmScale * atrScale;
        if (qty > 0 && qty < MIN_BINANCE_QTY) qty = MIN_BINANCE_QTY;
        if (qty > 0 && shortNet.qty + qty <= cfg.maxQty) {
          const fee = qty * mark * FEE_PCT / 100;
          wallet -= fee; totalFees += fee;
    ...
    // Setup #15
    if (ts - lastS15Ms >= SETUP15_COOLDOWN_MS) {
      ...
      if (fIdx >= 3) {
        ...
        if (s15Side) {
          ...
          if (q > 0 && cur + q <= SIMPLE_MAX_QTY_PER_SIDE) {
            ...
            setupCounts[`S15${s15Side[0]}`] = (setupCounts[`S15${s15Side[0]}`] ?? 0) + 1;
            const y = new Date(ts).toISOString().slice(0, 4);
            byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 }; byYear[y].entries++;
          }
        }
      ...
      if (vmAvg > 0 && vol / vmAvg >= SETUP16_VOL_RATIO) {
        ...
        if (s16Side) {
          ...
          if (q > 0 && cur + q <= SIMPLE_MAX_QTY_PER_SIDE) {
            ...
            entries++;
            setupCounts[`S16${s16Side[0]}`] = (setupCounts[`S16${s16Side[0]}`] ?? 0) + 1;
            const y = new Date(ts).toISOString().slice(0, 4);
            byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 }; byYear[y].entries++;
          }
        }
    ...
    // Setup #17 Scalp 5m (V2/V4)
    if (cfg.enableScalp17 && i >= 21) {
      const r5_now = rsi5[i]; const m20_now = ma20_5[i]; const s20_now = sd20_5[i];
      if (r5_now !== null && m20_now !== null && s20_now !== null && s20_now > 0) {
        ...
        if (s17Side) {
          ...
          if (q > 0 && cur + q <= SIMPLE_MAX_QTY_PER_SIDE) {
            ...
            entries++;
            setupCounts[`S17${s17Side[0]}`] = (setupCounts[`S17${s17Side[0]}`] ?? 0) + 1;
            const y = new Date(ts).toISOString().slice(0, 4);
            byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 }; byYear[y].entries++;
          }
        }
    ...
    // Setup #18 Aggressive BULL DCA (V3/V4)
    if (cfg.enableDCA18 && regime === "BULL" && ts - lastS18Ms >= SETUP18_COOLDOWN_MS) {
      // LONG khi RSI 15m < 50 + close > MA200 1d (confirm bull)
      const r15v = ind.rsi15[idx15c]; const m200dv = ind.ma200d[idx1dc];
      if (r15v !== null && r15v < 50 && m200dv !== null && c1d[idx1dc].close > m200dv) {
        let q = SETUP18_QTY * hwmScale * atrScale;
        if (q > 0 && q < MIN_BINANCE_QTY) q = MIN_BINANCE_QTY;
        const cur = simpleLongNet.qty;
        if (q > 0 && cur + q <= SIMPLE_MAX_QTY_PER_SIDE) {
          const fee = q * mark * FEE_PCT / 100;
          wallet -= fee; totalFees += fee;
          const nq = simpleLongNet.qty + q;
          simpleLongNet = { qty: nq, avg: (simpleLongNet.qty * simpleLongNet.avg + q * mark) / nq };
          // S18 KHÔNG có SL (DCA-style), only TP +5% + time stop 7d
          simpleTrades.push({
            id: `s18_${ts}`, kind: "S18", side: "LONG", entryPx: mark, qty: q,
            tpPx: mark * (1 + SETUP18_TP_PCT / 100),
            slPx: 0,  // no SL
            expireTs: ts + SETUP18_TIMESTOP_MS,
          });
          lastS18Ms = ts; entries++;
          setupCounts["S18L"] = (setupCounts["S18L"] ?? 0) + 1;
          const y = new Date(ts).toISOString().slice(0, 4);
          byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 }; byYear[y].entries++;
        }
      }
...
function main() {
  ...
  console.log("[v0438] Pre-computing indicators...");
  ...
  console.log("\n=== AGGRESSIVE SCENARIOS COMPARISON (3y, $100k) ===");
  console.log("Variant                 | ROI%   | DD%   | RA    | Entries | Closes | WR%   | R:R  | Exp/trade | Fees");
  console.log("-".repeat(118));
  for (const r of results) {
    console.log(`${r.name.padEnd(24)} | ${String(r.roi).padStart(6)} | ${String(r.maxDD).padStart(5)} | ${String(r.ra).padStart(5)} | ${String(r.entries).padStart(7)} | ${String(r.closes).padStart(6)} | ${String(r.wr).padStart(5)} | ${String(r.rr).padStart(4)} | ${String(r.exp).padStart(9)} | $${String(r.totalFees).padStart(5)}`);
  }
  ...
  for (const r of results) {
    console.log(`\n${r.name}:`);
    const sigs = Object.entries(r.setupCounts).sort(([, a]: any, [, b]: any) => b - a);
    for (const [sig, count] of sigs.slice(0, 15)) {
      console.log(`  ${sig.padEnd(20)} count=${count}`);
    }
  }

  writeFileSync(join(__dirname, "..", "assets", "backtest_h01_v0438_aggressive.json"), JSON.stringify(results, null, 2));
  console.log(`\nWritten assets/backtest_h01_v0438_aggressive.json`);
}

/**
 * backtest-hedge01-v0432-variants.ts — test 4 tuning variants song song.
 *
 * Baseline: v0.4.32 hiện tại (MAX_QTY 0.05, SL -12%, stacked add)
 *
 * Variants:
 *   A: SL -8% (tight cut)
 *   B: No stack (skip entry nếu side đã có position)
 *   C: Per-entry SL (mỗi ADD track riêng với SL -8%, tương tự Setup #11 cho ALL entries)
 *   D: No aggregate SL (về v0.4.27 style)
 *
 * Capital $100k, MAX_QTY 0.05, 3y data.
 */
...
interface Variant {
  name: string;
  aggregateSlPct: number | null;  // null = no SL
  noStack: boolean;                // true = skip entry nếu side đã có qty > 0
  perEntrySl: boolean;             // true = mỗi ADD track riêng SL như Setup #11
  perEntrySlPct: number;
}

const VARIANTS: Variant[] = [
  { name: "BASELINE_v0.4.32", aggregateSlPct: 12, noStack: false, perEntrySl: false, perEntrySlPct: 0 },
  { name: "A_SL_tight_8pct",  aggregateSlPct: 8,  noStack: false, perEntrySl: false, perEntrySlPct: 0 },
  { name: "B_no_stack",       aggregateSlPct: 12, noStack: true,  perEntrySl: false, perEntrySlPct: 0 },
  { name: "C_per_entry_SL_8", aggregateSlPct: null, noStack: false, perEntrySl: true, perEntrySlPct: 8 },
  { name: "D_no_aggregate_SL",aggregateSlPct: null, noStack: false, perEntrySl: false, perEntrySlPct: 0 },
];

interface Candle { time: number; open: number; high: number; low: number; close: number; volume?: number; }
interface Net { qty: number; avg: number; }
type Regime = "BULL" | "RANGE" | "BEAR";
interface TrackedTrade { id: string; entryPx: number; qty: number; tpPx: number; slPx: number; kind: "S11" | "AGG" | "DD" | "C2"; }
...
function runBacktest(c5: Candle[], c15: Candle[], c1h: Candle[], c1d: Candle[], c1w: Candle[], ind: Indicators, v: Variant): any {
  ...
  let haltedUntil = 0;
  let trackedLong: TrackedTrade[] = [];   // Setup #11 + variant C per-entry
  let trackedShort: TrackedTrade[] = [];  // Variant C cho SHORT
  ...
  for (let i = 0; i < c5.length; i++) {
    const bar5 = c5[i];
    const ts = bar5.time;
    const mark = bar5.close;
    idx15 = findIdx(c15, ts, idx15);
    idx1h = findIdx(c1h, ts, idx1h);
    idx1d = findIdx(c1d, ts, idx1d);
    idx1w = findIdx(c1w, ts, idx1w);
    const idx15c = idx15 - 1;
    const idx5c = i - 1;
    const idx1hc = idx1h - 1;
    const idx1dc = idx1d - 1;

    // Per-trade close (Setup #11 + variant C tracked trades) — LONG
    const newL: TrackedTrade[] = [];
    for (const t of trackedLong) {
    ...
    // Per-trade close — SHORT (variant C only)
    const newS: TrackedTrade[] = [];
    for (const t of trackedShort) {
      if (mark <= t.tpPx || mark >= t.slPx) {  // SHORT: TP khi mark < entry, SL khi mark > entry
        const fee = t.qty * mark * FEE_PER_SIDE_PCT / 100;
        const pnl = (t.entryPx - mark) * t.qty;
        wallet += pnl - fee; totalFees += fee;
        if (wallet > hwm) hwm = wallet;
        if (wallet < lowestWallet) lowestWallet = wallet;
        closes++;
        ...
        byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 };
        byYear[y].closes++; byYear[y].pnl += pnl - fee;
        shortNet = { qty: Math.max(0, shortNet.qty - t.qty), avg: shortNet.qty - t.qty > 0 ? shortNet.avg : 0 };
      } else newS.push(t);
    }
    trackedShort = newS;

    // Aggregate TP/SL — bỏ qua nếu variant C (per-entry SL only) hoặc D (no SL)
    if (longNet.qty > 0 && longNet.avg > 0) {
      const gain = (mark - longNet.avg) / longNet.avg * 100;
      const slTrig = v.aggregateSlPct !== null && gain <= -v.aggregateSlPct;
      if (gain >= TP_PCT || slTrig) {
    ...
    if (shortNet.qty > 0 && shortNet.avg > 0) {
      ...
      const slTrig = v.aggregateSlPct !== null && drop <= -v.aggregateSlPct;
      if (drop >= TP_PCT || slTrig) {
    ...
    let c2Fire = false;
    if (idx5c >= 22) {
      const last5 = c5[idx5c];
      const r5 = ind.rsi5[idx5c];
      if (r5 !== null && r5 >= 70) {
    ...
    // Variant B: skip nếu side đã có position
    const longSideEmpty = longNet.qty === 0;
    const shortSideEmpty = shortNet.qty === 0;

    if (!longCD && (!v.noStack || longSideEmpty)) {
      let qty = 0;
      if (lS >= 9) {
      ...
      if (qty > 0 && longNet.qty + qty <= MAX_QTY) {
        ...
        byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 };
        byYear[y].entries++;
        // Variant C: track riêng mỗi ADD với SL
        if (v.perEntrySl) {
          const tpPx = mark * (1 + TP_PCT / 100);
          const slPx = mark * (1 - v.perEntrySlPct / 100);
          trackedLong.push({ id: `agg_${ts}`, entryPx: mark, qty, tpPx, slPx, kind: "AGG" });
        }
      }
    }

    if (!s11CD && c2Fire && f4Pass && allowMomLong && (!v.noStack || longSideEmpty)) {
      ...
      if (scaledQty >= MIN_BINANCE_QTY && longNet.qty + scaledQty <= MAX_QTY) {
        ...
        lastSetup11Ms = ts;
        trackedLong.push({ id: tradeId, entryPx: mark, qty: scaledQty, tpPx, slPx, kind: "S11" });
        entries++;
        const y = new Date(ts).toISOString().slice(0, 4);
        byYear[y] = byYear[y] ?? { entries: 0, closes: 0, pnl: 0 };
        byYear[y].entries++;
    ...
    if (sS >= 9 && !shortCD && allowShortSide && (!v.noStack || shortSideEmpty)) {
      ...
      if (weeklyDown) {
        ...
        if (qty > 0 && shortNet.qty + qty <= MAX_QTY) {
          ...
          if (v.perEntrySl) {
            const tpPx = mark * (1 - TP_PCT / 100);
            const slPx = mark * (1 + v.perEntrySlPct / 100);
            trackedShort.push({ id: `aggS_${ts}`, entryPx: mark, qty, tpPx, slPx, kind: "AGG" });
          }
        }
      }
    }
  ...
  return {
    variant: v.name, wallet: Math.round(wallet), hwm: Math.round(hwm), lowestWallet: Math.round(lowestWallet),
    roi: +roi.toFixed(2), maxDD: +maxDD.toFixed(2), riskAdj: +riskAdj.toFixed(2),
...
function main() {
  console.log("[variants] Loading caches...");
  ...
  console.log("[variants] Pre-computing indicators...");
  ...
  const ind: Indicators = {
    ...
    ma20_15: ma20,
    sd20_15: calcStdev(close15, 20, ma20),
  ...
  console.log("\n=== VARIANT COMPARISON (capital $100k, MAX_QTY 0.05, 3y) ===");
  console.log("Variant              | ROI%    | DD%   | RiskAdj | Entries | Closes | WR%   | AvgWin  | AvgLoss  | R:R  | Exp/trade");
  console.log("-".repeat(125));
  for (const r of results) {
    console.log(`${r.variant.padEnd(20)} | ${String(r.roi).padStart(7)} | ${String(r.maxDD).padStart(5)} | ${String(r.riskAdj).padStart(7)} | ${String(r.entries).padStart(7)} | ${String(r.closes).padStart(6)} | ${String(r.wr).padStart(5)} | ${String(r.avgWin).padStart(7)} | ${String(r.avgLoss).padStart(8)} | ${String(r.rr).padStart(4)} | ${String(r.expectancy).padStart(8)}`);
  }
  ...
  for (const r of results) {
    ...
    console.log(`${r.variant.padEnd(20)} | ${String(y23).padStart(7)} | ${String(y24).padStart(7)} | ${String(y25).padStart(7)} | ${String(y26).padStart(7)}`);
  }

  writeFileSync(join(__dirname, "..", "assets", "backtest_h01_v0432_variants.json"), JSON.stringify(results, null, 2));
  console.log(`\nWritten assets/backtest_h01_v0432_variants.json`);
}

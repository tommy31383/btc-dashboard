#!/usr/bin/env python3
"""autoloop-r89-last-explore.py — Round 89: Final exploration with R87 config
R88 confirmed: ALL params optimal, ceiling Sh+2.48 DD4.6%

R89 new directions not yet tried with R87:
  A: BTC ADX threshold re-tune (might be different with all improvements)
  B: BTC SL ratio sweep — INIT vs TRAIL ratio (current 2.78/3.28 = diff 0.50)
  C: vol_mult for BTC (currently 1.4, was optimal without all improvements)
  D: Turtle DX sweep with R87 (was DX=14, re-confirm)
  E: SOL volume filter OFF — what if we remove vol filter for SOL entirely?
"""
...
def run_h01(cache, adx_thresh=18, sl_init=3.0, sl_trail=3.5, sl_trans=16,
             ...
             skip_months=None, donchian_lb=18, no_vol_filter=False):
    H.CACHE=cache
    ...
    def vol_pass(i):
        if no_vol_filter: return True  # skip vol filter
...
print(); print("="*100); print("=== R89: Last exploration ===\n")

# A: BTC ADX thresh sweep
print("━"*100); print("A: BTC ADX threshold sweep with R87 config")
for adxt in [15,16,17,18,19,20,22]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",adxt,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC_ADX={adxt}",[sB_,sS,sTB14],extra=f"n={nt}")

# B: BTC SL ratio sweep (init vs trail gap)
print(f"\n{'━'*100}"); print("B: BTC SL ratio — init vs trail gap (current=0.50)")
for si,gap in [(2.78,0.3),(2.78,0.4),(2.78,0.5),(2.78,0.6),(2.78,0.7)]:
    st=si+gap
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=si,sl_trail=st,skip_months={8},donchian_lb=18,vol_ma=16)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  SL {si}/{st:.2f} (gap={gap})",[sB_,sS,sTB14],extra=f"n={nt}")

# C: BTC vol_mult sweep
print(f"\n{'━'*100}"); print("C: BTC vol_mult sweep with R87 (current=1.4)")
for vm in [1.2,1.3,1.4,1.5,1.6]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16,vol_mult=vm)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC vol_mult={vm}",[sB_,sS,sTB14],extra=f"n={nt}")

# D: Turtle DX re-confirm
print(f"\n{'━'*100}"); print("D: Turtle DX sweep with R87 config (was DX=14)")
for DX in [12,13,14,15,16]:
    tur_=run_turtle(DX); sTur_=[tur_.get(m,0.0) for m in cal3]
    pr(f"  Turtle DX={DX}",[sB,sS,sTur_])

# E: SOL vol filter OFF
print(f"\n{'━'*100}"); print("E: SOL vol filter removal (no vol filter)")
moS_novol,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=20,donchian_lb=20,vol_ma=16,atr_break_mult=1.5,no_vol_filter=True)
sS_nv=[moS_novol.get(m,0.0) for m in cal3]
pr(f"  SOL no-vol-filter",[sB,sS_nv,sTB14],extra=f"n={nt}")
pr(f"  SOL with vol_filter (baseline)",[sB,sS,sTB14])

print(f"\n{'='*100}")
print(f"R89 SUMMARY — Ceiling Sh{sh_base:+.3f}")
print(f"  R87 = R89 FINAL CONFIRMED CONFIG:")

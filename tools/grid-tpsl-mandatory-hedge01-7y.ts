/**
 * grid-tpsl-mandatory-hedge01-7y.ts (anh Tommy 2026-05-14)
 *
 * Re-grid hedge01 + C2 v0.4.27 với SL MANDATORY (loại survivorship bias "no SL").
 * Constraint: TP >= SL (R:R ratio reasonable, không hold lâu hơn rủi ro).
 * C2 per-position SL cũng MANDATORY (3-10%).
 *
 * Grid:
 *   - TP [7, 8, 10, 12, 15, 18, 20]
 *   - SL [5, 7, 8, 10, 12, 15, 18, 20]  (chỉ giữ SL ≤ TP)
 *   - C2SL [3, 5, 7, 10]
 *
 * Total valid: 35 (TP,SL) × 4 C2SL = 140 combos.
 *
 * Sau khi rank by RiskAdj, top 5 → train/test 70/30 chronological split để
 * verify combo retain lift trên unseen 2024-2026 data (≈30%).
 */
...
interface Result {
  tp: number; sl: number; c2sl: number; rr: number;
  ...
  yearsPositive: number; yearsNegative: number;
}
  ...
  for (let i = iStart; i < iEnd; i++) {
    ...
    if (longQty > 0 && longAvg > 0) {
      ...
      if (gain >= tpPct) {
        ...
        wallet += np; closes++; tpHits++;
        const year = new Date(ts).getUTCFullYear();
        perYearPnl[year] = (perYearPnl[year] || 0) + np;
        longQty = 0; longAvg = 0; c2Entries.length = 0;
      } else if (gain <= -slPct) {
        const realized = longQty * (price - longAvg);
        const fee = longQty * price * (FEE_PER_SIDE_PCT/100);
        const np = realized - fee;
        wallet += np; closes++; slHits++;
        const year = new Date(ts).getUTCFullYear();
        perYearPnl[year] = (perYearPnl[year] || 0) + np;
        longQty = 0; longAvg = 0; c2Entries.length = 0;
      }
    ...
    // C2 per-position SL (mandatory, c2SlPct > 0)
    if (longQty > 0 && c2Entries.length > 0) {
      for (let k = c2Entries.length - 1; k >= 0; k--) {
        const e = c2Entries[k];
        const loss = (price - e.entryPrice) / e.entryPrice * 100;
        if (loss <= -c2SlPct) {
          const realized = e.qty * (price - longAvg);
    ...
    if (c2Sig[i] === 1) {
      ...
      if (cool) {
        ...
        wallet -= fee;
        c2Entries.push({ qty: C2_QTY, entryPrice: price });
      }
    }
...
function main() {
  ...
  console.log("[grid-mandatory] Computing C2 signal on 5m...");
  ...
  const tpGrid = [7, 8, 10, 12, 15, 18, 20];
  const slGrid = [5, 7, 8, 10, 12, 15, 18, 20];
  const c2SlGrid = [3, 5, 7, 10];

  // Build valid combos: TP >= SL (R:R ≥ 1:1, hợp lý)
  const combos: Array<[number, number, number]> = [];
  for (const tp of tpGrid) {
    for (const sl of slGrid) {
      if (sl > tp) continue;
      for (const c2sl of c2SlGrid) {
        combos.push([tp, sl, c2sl]);
      }
    }
  }
  console.log(`[grid-mandatory] Running ${combos.length} valid combos (TP≥SL, SL/C2SL mandatory)...`);

  const t0 = Date.now();
  const results: Result[] = [];
  for (let n = 0; n < combos.length; n++) {
    const [tp, sl, c2sl] = combos[n];
    const r = simulate(c5, 0, c5.length, sigByTs, c2Sig, weekClosedRedAt, tp, sl, c2sl);
    results.push(r);
    if ((n+1) % 20 === 0) {
      const dt = (Date.now() - t0) / 1000;
      console.log(`  ${n+1}/${combos.length} done · ${dt.toFixed(1)}s · last: TP${tp}/SL${sl}/C2SL${c2sl} ROI ${r.roi.toFixed(1)}% DD ${r.ddPct.toFixed(1)}%`);
    }
  }
  console.log(`[grid-mandatory] Full 7y done in ${((Date.now()-t0)/1000).toFixed(1)}s\n`);

  // Rank by RiskAdj (no LIQ + stability ≥ 5/7 năm dương)
  const valid = results.filter(r => !r.liquidated && r.yearsPositive >= 5);
  const top = [...valid].sort((a, b) => b.riskAdj - a.riskAdj).slice(0, 10);

  console.log(`=== TOP 10 BY RISK-ADJUSTED ROI (SL mandatory, stability ≥ 5/7) ===`);
  console.log(`Rank | TP  | SL  | C2SL | R:R   | ROI%    | DD%   | RiskAdj | Closes (TP/SL/C2SL) | Yrs+ | C2-ADDs`);
  console.log(`-----|-----|-----|------|-------|---------|-------|---------|---------------------|------|--------`);
  top.forEach((r, i) => {
    console.log(`${String(i+1).padStart(4)} | ${String(r.tp).padStart(3)} | ${String(r.sl).padStart(3)} | ${String(r.c2sl).padStart(4)} | ${r.rr.toFixed(2)} | ${(r.roi>=0?"+":"")}${r.roi.toFixed(2).padStart(7)} | ${r.ddPct.toFixed(1).padStart(5)} | ${r.riskAdj.toFixed(2).padStart(7)} | ${String(r.closes).padStart(3)} (${r.tpHits}/${r.slHits}/${r.c2SlHits}) | ${r.yearsPositive}/7 | ${String(r.c2Adds).padStart(6)}`);
  });
  ...
  console.log(`Train: ${splitIdx} bars (~Jan 2019 → ${splitDate})`);
  console.log(`Test:  ${c5.length - splitIdx} bars (${splitDate} → May 2026)\n`);
  console.log(`Combo                | TRAIN ROI / DD / RiskAdj | TEST ROI / DD / RiskAdj | DECAY?`);
  console.log(`---------------------|--------------------------|-------------------------|--------`);
  const splitResults: Array<{combo: Result; train: Result; test: Result}> = [];
  for (const r of top.slice(0, 5)) {
    const train = simulate(c5, 0, splitIdx, sigByTs, c2Sig, weekClosedRedAt, r.tp, r.sl, r.c2sl);
    const test = simulate(c5, splitIdx, c5.length, sigByTs, c2Sig, weekClosedRedAt, r.tp, r.sl, r.c2sl);
    splitResults.push({ combo: r, train, test });
    const liftRetain = test.riskAdj / (train.riskAdj || 1);
    const decay = liftRetain < 0.7 ? "⚠️ HEAVY" : liftRetain < 0.9 ? "moderate" : "OK";
    const label = `TP${r.tp}/SL${r.sl}/C2SL${r.c2sl}`;
    console.log(`${label.padEnd(20)} | ${(train.roi>=0?"+":"")}${train.roi.toFixed(1).padStart(5)}% / ${train.ddPct.toFixed(1).padStart(4)}% / ${train.riskAdj.toFixed(2).padStart(5)} | ${(test.roi>=0?"+":"")}${test.roi.toFixed(1).padStart(5)}% / ${test.ddPct.toFixed(1).padStart(4)}% / ${test.riskAdj.toFixed(2).padStart(5)} | ${decay}`);
  }

  // Per-year breakdown top 1
  const best = top[0];
  console.log(`\n=== TOP 1 PER-YEAR PnL (TP${best.tp}/SL${best.sl}/C2SL${best.c2sl}) ===`);
  ...
  console.log(`\nValid combos (no LIQ + ≥5/7 yr+): ${valid.length}/${results.length}`);

  writeFileSync(join(__dirname, "..", "assets", "grid_tpsl_mandatory_hedge01_7y.json"), JSON.stringify({ top10: top, splitResults, all: results, splitDate }, null, 2));
  console.log(`\n[grid-mandatory] ✅ Done — saved assets/grid_tpsl_mandatory_hedge01_7y.json`);
}

#!/usr/bin/env python3
"""autoloop-r76-ema150.py — Round 76: EMA1h=150 verify + combine
R75 find: EMA1h=150 → Sh+2.31 (+0.054 ★★★, DD5.3%, n=142 vs n=138)

  A: EMA1h fine sweep 120-180 — confirm 150 peak
  B: EMA1h=150 robustness — holdout years
  C: EMA1h=150 + all R74 config (BTC78+SOL29+DX14+T1.2+Aug-skip)
  D: EMA1h SOL (apply shorter EMA to SOL too?)
  E: EMA1h=150 vs 200 at different SL configs
"""
...
moB_200,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=200)
moB_150,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=150)
...
sB200=[moB_200.get(m,0.0) for m in cal3]; sB150=[moB_150.get(m,0.0) for m in cal3]
...
sh_base,_=pr("R74 BASELINE (EMA200)",[sB200,sS29,sTB14])
pr("  EMA150 (R75 find)",[sB150,sS29,sTB14])
print(); print("="*100); print("=== R76: EMA1h=150 verify ===\n")

# A: EMA1h fine sweep
print("━"*100); print("A: EMA1h fine sweep 120-180 — confirm 150 peak")
best_ema=200; best_ema_sh=sh_base; best_moB=moB_200
for ema_p in [100,110,120,130,140,150,160,170,180,200,250]:
    ...
    sh,md=pr(f"  EMA1h={ema_p}",[sB_,sS29,sTB14],extra=f"n={nt}")
    if sh>best_ema_sh: best_ema_sh=sh; best_ema=ema_p; best_moB=moB_
print(f"\n  → Best EMA1h={best_ema} Sh{best_ema_sh:+.3f}")

# B: EMA1h=150 robustness
print(f"\n{'━'*100}"); print("B: EMA1h=150 robustness — holdout years")
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB200_ho=[moB_200.get(m,0.0) for m in cal_ho]; sB150_ho=[moB_150.get(m,0.0) for m in cal_ho]
    sS_ho=[moS29.get(m,0.0) for m in cal_ho]; sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_200,_,_,_,_=bk([sB200_ho,sS_ho,sTB_ho],cal=cal_ho)
    sh_150,_,_,_,_=bk([sB150_ho,sS_ho,sTB_ho],cal=cal_ho)
    robust="✓ ROBUST" if sh_150>sh_200 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: EMA200 Sh{sh_200:+.2f} → EMA150 Sh{sh_150:+.2f} delta{sh_150-sh_200:>+.3f} {robust}")

# C: EMA150 + all R74 config
print(f"\n{'━'*100}"); print("C: EMA150 combine variations")
sBbest=[best_moB.get(m,0.0) for m in cal3]
for lbl,sB_,w in [
    ("EMA150 BTC + SOL_orig + T1.2",sB150,W),
    ("EMA150 BTC + SOL_orig + T1.3",sB150,[1,1,1.3]),
    ("EMA150 BTC + SOL_orig + T1.0",sB150,[1,1,1.0]),
    (f"EMA{best_ema} BTC (best) + SOL_orig + T1.2",sBbest,W),
]:
    pr(f"  {lbl}",[sB_,sS29,sTB14],weights=w)

# D: EMA SOL sweep
print(f"\n{'━'*100}"); print("D: SOL EMA1h filter (SOL has own EMA200-1h filter?)")
moS_150,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,ema_1h=150)
moS_100,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,ema_1h=100)
sS150=[moS_150.get(m,0.0) for m in cal3]; sS100=[moS_100.get(m,0.0) for m in cal3]
pr("  BTC150 + SOL200",[sB150,sS29,sTB14])
pr("  BTC150 + SOL150",[sB150,sS150,sTB14])
pr("  BTC150 + SOL100",[sB150,sS100,sTB14])
pr("  BTC200 + SOL150",[sB200,sS150,sTB14])

# E: EMA150 with different BTC SL (maybe SL interacts with EMA)
print(f"\n{'━'*100}"); print("E: EMA150 + BTC SL sweep")
for si,st in [(2.70,3.20),(2.75,3.25),(2.78,3.28),(2.80,3.30),(2.82,3.32),(2.85,3.35),(2.90,3.40)]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=si,sl_trail=st,skip_months={8},ema_1h=150)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  EMA150 SL{si}/{st}",[sB_,sS29,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R76 SUMMARY — Ceiling Sh{sh_base:+.3f}")
print(f"  Best EMA: {best_ema} → Sh{best_ema_sh:+.3f}")
print(f"  EMA1h=150 ROBUST? → check section B")

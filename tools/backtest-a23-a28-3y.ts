/**
 * backtest-a23-a28-3y.ts — Test 4 new improvements on v0.4.45 (A19+A22+A20) baseline
 *
 * A23: ADX-weighted qty — ADX > 30 → qty × 1.5 (amplify high-confidence trends)
 * A25: Pyramid winners — unrealized P% > +1% → add 0.5× more once (compound winners)
 * A26: Transition window sweep — 24h vs 72h (current 48h default)
 * A28: 3-stage SL ladder — ATR×6 first 1h → ATR×4 1-48h → ATR×3 post-48h
 *      (always has SL, gradually tighter)
 *
 * A27 funding arb separate — cần new setup architecture (Setup #15 simple bucket)
 * A24 multi-symbol separate — cần fetch ETH/SOL data
 */
...
// v0.4.45 baseline shipped SL
const DEFAULT_INITIAL_SL = 4;
const DEFAULT_TRAILING_SL = 3;
const DEFAULT_TRANSITION_MS = 48 * 60 * 60_000;

// A23
const A23_ADX_STRONG = 30;
const A23_QTY_MULT = 1.5;
// A25
const A25_PYRAMID_TRIGGER_PCT = 1.0;  // +1% unrealized
const A25_PYRAMID_QTY_MULT = 0.5;
// A28 3-stage
const A28_STAGE1_SL = 6; const A28_STAGE1_MS = 1 * 60 * 60_000;       // first 1h
const A28_STAGE2_SL = 4; const A28_STAGE2_MS = 48 * 60 * 60_000;      // 1-48h
const A28_STAGE3_SL = 3;                                              // 48h+
...
interface VariantCfg {
  name: string;
  a23WeightedQty: boolean;
  a25Pyramid: boolean;
  a26TransitionHrs: number;
  a28ThreeStage: boolean;
}

const VARIANTS: VariantCfg[] = [
  { name: "BASELINE_v045",   a23WeightedQty: false, a25Pyramid: false, a26TransitionHrs: 48, a28ThreeStage: false },
  { name: "A23_weightedQty", a23WeightedQty: true,  a25Pyramid: false, a26TransitionHrs: 48, a28ThreeStage: false },
  { name: "A25_pyramid",     a23WeightedQty: false, a25Pyramid: true,  a26TransitionHrs: 48, a28ThreeStage: false },
  { name: "A26_24h",         a23WeightedQty: false, a25Pyramid: false, a26TransitionHrs: 24, a28ThreeStage: false },
  { name: "A26_72h",         a23WeightedQty: false, a25Pyramid: false, a26TransitionHrs: 72, a28ThreeStage: false },
  { name: "A28_3stageSL",    a23WeightedQty: false, a25Pyramid: false, a26TransitionHrs: 48, a28ThreeStage: true  },
  { name: "A_ALL_combo",     a23WeightedQty: true,  a25Pyramid: true,  a26TransitionHrs: 48, a28ThreeStage: true  },
];

interface TrendTrade {
  id: string; kind: string; side: "LONG" | "SHORT";
  entryPx: number; qty: number; hwm: number; lwm: number; slPx: number; atrEntry: number; entryTs: number;
  avgEntry: number;  // for pyramid tracking
  pyramided: boolean;
}

function slMultForHold(heldMs: number, cfg: VariantCfg): number {
  if (cfg.a28ThreeStage) {
    if (heldMs < A28_STAGE1_MS) return A28_STAGE1_SL;     // 6
    if (heldMs < A28_STAGE2_MS) return A28_STAGE2_SL;     // 4
    return A28_STAGE3_SL;                                  // 3
  }
  const transitionMs = cfg.a26TransitionHrs * 60 * 60_000;
  if (heldMs < transitionMs) return DEFAULT_INITIAL_SL;   // 4
  return DEFAULT_TRAILING_SL;                              // 3
}
...
function run(c5: Candle[], c1h: Candle[], c4h: Candle[], c1d: Candle[], ind: any, cfg: VariantCfg, startIdx: number, endIdx: number): any {
  ...
  let entries = 0, pyramidAdds = 0, closes = 0, wins = 0, losses = 0, sumWin = 0, sumLoss = 0;
  ...
  for (let i = startIdx; i < c5.length && i < endIdx; i++) {
    const bar = c5[i]; const ts = bar.time; const mark = bar.close;

    // SL chase + close + A25 pyramid check
    const newTrend: TrendTrade[] = [];
    for (const t of trendTrades) {
      const heldMs = ts - t.entryTs;
      const slMult = slMultForHold(heldMs, cfg);
      if (t.side === "LONG") {
        if (mark > t.hwm) { t.hwm = mark; t.slPx = t.hwm - t.atrEntry * slMult; }
        else if (heldMs >= cfg.a26TransitionHrs * 60 * 60_000 && !cfg.a28ThreeStage) {
          const targetSL = t.hwm - t.atrEntry * DEFAULT_TRAILING_SL;
          if (targetSL > t.slPx) t.slPx = targetSL;
        } else if (cfg.a28ThreeStage && heldMs >= A28_STAGE2_MS) {
          const targetSL = t.hwm - t.atrEntry * A28_STAGE3_SL;
          if (targetSL > t.slPx) t.slPx = targetSL;
        }
      } else {
        if (mark < t.lwm) { t.lwm = mark; t.slPx = t.lwm + t.atrEntry * slMult; }
        else if (heldMs >= cfg.a26TransitionHrs * 60 * 60_000 && !cfg.a28ThreeStage) {
          const targetSL = t.lwm + t.atrEntry * DEFAULT_TRAILING_SL;
          if (targetSL < t.slPx) t.slPx = targetSL;
        } else if (cfg.a28ThreeStage && heldMs >= A28_STAGE2_MS) {
          const targetSL = t.lwm + t.atrEntry * A28_STAGE3_SL;
          if (targetSL < t.slPx) t.slPx = targetSL;
        }
      }

      // A25 PYRAMID: nếu unrealized P% > +1%, add 0.5× more một lần
      if (cfg.a25Pyramid && !t.pyramided) {
        const upPct = t.side === "LONG" ? (mark - t.avgEntry) / t.avgEntry * 100 : (t.avgEntry - mark) / t.avgEntry * 100;
        if (upPct >= A25_PYRAMID_TRIGGER_PCT) {
          const addQty = t.qty * A25_PYRAMID_QTY_MULT;
          const cur = t.side === "LONG" ? trendLongNet.qty : trendShortNet.qty;
          if (cur + addQty <= TREND_MAX_QTY_PER_SIDE) {
            const fee = addQty * mark * FEE_PCT / 100;
            wallet -= fee;
            t.avgEntry = (t.avgEntry * t.qty + mark * addQty) / (t.qty + addQty);
            t.qty += addQty;
            // Update bucket
            const net = t.side === "LONG" ? trendLongNet : trendShortNet;
            const nq = net.qty + addQty;
            const newNet = { qty: nq, avg: (net.qty * net.avg + addQty * mark) / nq };
            if (t.side === "LONG") trendLongNet = newNet; else trendShortNet = newNet;
            t.pyramided = true;
            pyramidAdds++;
          }
        }
      ...
      if (exit) {
        ...
        const pnl = (t.side === "LONG" ? mark - t.avgEntry : t.avgEntry - mark) * t.qty;
        const net = pnl - fee;
        wallet += net;
    ...
    // A19+A22 filters (baseline always on)
    const adxVal = ind.adx14_4h[idx4hc];
    const adxPrev = ind.adx14_4h[idx4hc - 1];
    const adxPass = adxVal !== null && adxVal > ADX_THRESHOLD && adxPrev !== null && adxPrev > ADX_THRESHOLD;
    ...
    const vol4hPass = (() => {
      ...
      return ma !== null && ma > 0 && vol >= ma * VOL_MULT;
    })();
    ...
    const trendEnter = (kind: string, side: "LONG" | "SHORT", baseQty: number, lastTsRef: { v: number }, cdMs: number) => {
      ...
      // A23 weighted qty
      let qty = baseQty;
      if (cfg.a23WeightedQty && adxVal !== null && adxVal >= A23_ADX_STRONG) qty *= A23_QTY_MULT;
      const cur = side === "LONG" ? trendLongNet.qty : trendShortNet.qty;
      if (cur + qty > TREND_MAX_QTY_PER_SIDE) return;
      // Initial SL based on variant (A28 stage 1 = ATR×6, else ATR×4)
      const initialSL = cfg.a28ThreeStage ? A28_STAGE1_SL : DEFAULT_INITIAL_SL;
      const slPx = side === "LONG" ? mark - atrVal4h * initialSL : mark + atrVal4h * initialSL;
      ...
      trendTrades.push({ id: `${kind}_${side}_${ts}`, kind, side, entryPx: mark, qty, hwm: mark, lwm: mark, slPx, atrEntry: atrVal4h, entryTs: ts, avgEntry: mark, pyramided: false });
  ...
  return { entries, pyramidAdds, closes, wr: +wr.toFixed(1), roi: +roi.toFixed(2), maxDD: +maxDD.toFixed(2), ra: +ra.toFixed(2), rr: avgLoss < 0 ? +(avgWin / -avgLoss).toFixed(2) : 0, maxConsecLosses, ddDurationDays: +(maxDDDurationMs / 86_400_000).toFixed(1), byYear, byDir, bySetup, stab: `${stab}/${total}` };
}

function main() {
  console.log("[bt-a23-a28] Loading 3y...");
  ...
  console.log("[bt-a23-a28] Pre-computing indicators...");
  ...
  console.log("[bt-a23-a28] Running variants...");
  const results: any[] = [];
  for (const cfg of VARIANTS) {
    const r = run(c5, c1h, c4h, c1d, ind, cfg, startIdx, c5.length);
    results.push({ scenario: cfg.name, ...r });
    console.log(`${cfg.name.padEnd(22)} | RA ${r.ra} | ROI ${r.roi}% | DD ${r.maxDD}% | Entries ${r.entries} (+${r.pyramidAdds} pyramid) | WR ${r.wr}% | R:R ${r.rr} | Stab ${r.stab}`);
  }

  console.log("\n=== SUMMARY sorted by RA ===");
  results.sort((a, b) => b.ra - a.ra);
  console.log("Scenario             | RA   | ROI%  | DD%  | Entries | Pyramid | WR%   | R:R  | Stab");
  console.log("-".repeat(100));
  for (const r of results) {
    console.log(`${r.scenario.padEnd(20)} | ${String(r.ra).padStart(4)} | ${String(r.roi).padStart(5)} | ${String(r.maxDD).padStart(4)} | ${String(r.entries).padStart(7)} | ${String(r.pyramidAdds).padStart(7)} | ${String(r.wr).padStart(5)} | ${String(r.rr).padStart(4)} | ${r.stab.padStart(4)}`);
  }
  ...
  for (const r of results.slice(0, 4)) {
    ...
    for (const [s, d] of Object.entries(r.byDir)) {
      ...
      console.log(`  ${s.padEnd(6)} n=${String(dd.entries).padStart(4)} WR ${wr2.toFixed(1)}% PnL $${dd.pnl.toFixed(0)}`);
    }
    for (const [k, d] of Object.entries(r.bySetup).sort()) {
      const dd = d as any; const wr2 = dd.entries > 0 ? dd.wins / dd.entries * 100 : 0;
      console.log(`  ${k.padEnd(6)} n=${String(dd.entries).padStart(4)} WR ${wr2.toFixed(1)}% PnL $${dd.pnl.toFixed(0)}`);
    }
  }

  writeFileSync(join(__dirname, "..", "assets", "backtest_a23_a28_3y.json"), JSON.stringify(results, null, 2));
  console.log("\nWritten assets/backtest_a23_a28_3y.json");
}

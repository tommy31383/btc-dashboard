/**
 * backtest-5mall-stack-sweep-3y.ts
 *
 * Sweep `stackMaxPerSide` = 50 / 75 / 100 / 200 cho cả 3 preset 5m ALL hiện tại
 * (WHALE / EAGLE / TOMI). TOMI tắt trailing → fixed TP4/SL4 (anh Tommy yêu cầu).
 *
 * 3 preset × 4 stack = 12 combo. Output: bảng compare + JSON + HTML report.
 *
 * Usage:
 *   npx tsx tools/backtest-5mall-stack-sweep-3y.ts
 *   npx tsx tools/backtest-5mall-stack-sweep-3y.ts --years=1
 */
...
const STACK_SIZES = [50, 75, 100, 200];
...
// 3 preset hiện tại (mirror utils/all5mAccount.ts v4.8.22, TURTLE đã thay bằng TOMI)
// stackMaxPerSide ở đây là PLACEHOLDER — sẽ override theo STACK_SIZES.
const BASE_PRESETS: PresetDef[] = [
  ...
  },
  {
    ...
    // Anh Tommy: bỏ trailing → fixed TP4/SL4
    tpPct: 4, slPct: 4,
...
function genHtml(results: any[], years: number, range: { from: number; to: number }): string {
  ...
  // Sort by NET descending for ranking
  const sorted = [...results].sort((a, b) => b.netUsd - a.netUsd);
  const winner = sorted[0];
  const safest = [...results].sort((a, b) => a.maxDrawdownPct - b.maxDrawdownPct)[0];

  const rows = sorted.map((r, i) => {
    const trendBadge = r.equityTrend === "UP" ? "🟢 UP" : r.equityTrend === "DOWN" ? "🔴 DOWN" : "⚪ FLAT";
    const isWinner = r === winner ? " 🏆" : "";
    const isSafe = r === safest ? " 🛡️" : "";
    return `<tr>
      <td>${i + 1}${isWinner}${isSafe}</td>
      ...
      <td>${trendBadge}</td>
      <td>${r.avgHoldBars.toFixed(1)}b</td>
    </tr>`;
  }).join("\n");

  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>5m ALL Stack Sweep ${years}y</title>
<style>
body { font-family: -apple-system, sans-serif; background: #0a0a1a; color: #e8e8f0; padding: 24px; max-width: 1400px; margin: 0 auto; }
h1 { color: #ffd700; }
.meta { color: #888; margin-bottom: 24px; font-size: 13px; }
table { width: 100%; border-collapse: collapse; margin-top: 16px; font-size: 14px; }
th, td { border: 1px solid #2a2a40; padding: 8px 12px; text-align: right; }
th:nth-child(2), td:nth-child(2) { text-align: left; }
th { background: #1a1a2e; color: #ffd700; }
tr:nth-child(even) { background: #11111e; }
tr:hover { background: #222238; }
...
<body>
<h1>🎯 5m ALL — Stack Size Sweep (${years}y backtest)</h1>
<div class="meta">
  Period: ${fromStr} → ${toStr} · Capital $${INITIAL_CAPITAL} · Margin $${MARGIN_PER_TRADE}×${LEVERAGE}x · Fee ${FEE_PER_SIDE_PCT}%/side<br>
  3 PRESET (WHALE / EAGLE / TOMI) × 4 STACK [50/75/100/200] = 12 combo · TOMI trailing OFF (fixed TP4/SL4)
</div>

<table>
<thead><tr>
  <th>RANK</th><th>PRESET</th><th>STACK</th><th>TRADES</th><th>WR</th>
  <th>NET $</th><th>MAX DD $</th><th>DD %</th><th>PF</th><th>TREND</th><th>AVG HOLD</th>
</tr></thead>
<tbody>${rows}</tbody>
</table>

<div class="legend">
  <b>🏆 WINNER</b> = highest NET · <b>🛡️ SAFEST</b> = lowest DD% · <b>TREND</b> = equity curve slope (UP/FLAT/DOWN)<br>
  PF = Profit Factor (gross win / gross loss). PF > 1 = profitable, > 2 = strong edge.
</div>
...
(async () => {
  ...
  console.log(`\n=== BACKTEST 5m ALL — STACK SIZE SWEEP (${YEARS}y) ===`);
  console.log(`3 preset × ${STACK_SIZES.length} stack [${STACK_SIZES.join("/")}] = ${BASE_PRESETS.length * STACK_SIZES.length} combo`);
  console.log(`Capital=$${INITIAL_CAPITAL} margin=$${MARGIN_PER_TRADE}×${LEVERAGE}x fee=${FEE_PER_SIDE_PCT}%/side`);
  console.log(`TOMI trailing: OFF (fixed TP4/SL4 — anh Tommy)\n`);
  ...
  for (const base of BASE_PRESETS) {
    for (const stack of STACK_SIZES) {
      const preset: PresetDef = { ...base, stackMaxPerSide: stack };
      process.stdout.write(`${base.emoji} ${base.key} stack=${stack}...`);
      const t0 = Date.now();
  ...
  console.log(`\n${"─".repeat(110)}`);
  console.log(
    `${"PRESET".padEnd(10)} ${pad("STACK", 6)} ${pad("TRADES", 8)} ${pad("WR%", 7)} ` +
    `${pad("NET $", 12)} ${pad("MAX DD $", 10)} ${pad("DD%", 7)} ${pad("PF", 7)} ${pad("TREND", 7)} ${pad("HOLD", 8)}`
  );
  console.log(`${"─".repeat(110)}`);

  // Sort by NET descending for display
  const sorted = [...results].sort((a, b) => b.netUsd - a.netUsd);
  for (const r of sorted) {
    console.log(
      `${(r.emoji + " " + r.preset).padEnd(10)} ` +
  ...
  const winner = sorted[0];
  const safest = [...results].sort((a, b) => a.maxDrawdownPct - b.maxDrawdownPct)[0];
  console.log(`\n🏆 WINNER (highest NET): ${winner.emoji} ${winner.preset} stack=${winner.stackMaxPerSide} → NET $${winner.netUsd.toFixed(0)} (DD ${winner.maxDrawdownPct.toFixed(1)}%)`);
  console.log(`🛡️ SAFEST (lowest DD%): ${safest.emoji} ${safest.preset} stack=${safest.stackMaxPerSide} → DD ${safest.maxDrawdownPct.toFixed(1)}% (NET $${safest.netUsd.toFixed(0)})`);

  // Per-preset best stack
  console.log(`\n📌 BEST STACK PER PRESET:`);
  for (const base of BASE_PRESETS) {
    const subset = results.filter((r) => r.preset === base.key);
    const best = subset.reduce((a, b) => (b.netUsd > a.netUsd ? b : a));
    console.log(`   ${base.emoji} ${base.key}: stack=${best.stackMaxPerSide} → NET $${best.netUsd.toFixed(0)} (DD ${best.maxDrawdownPct.toFixed(1)}%, ${best.total} trades)`);
  }
  ...
  const out = {
    ...
    stackSizes: STACK_SIZES,
    presets: BASE_PRESETS.map((p) => ({ key: p.key, label: p.label, emoji: p.emoji, tpPct: p.tpPct, slPct: p.slPct })),
    results,
    winner: { preset: winner.preset, stack: winner.stackMaxPerSide, netUsd: winner.netUsd },
    safest: { preset: safest.preset, stack: safest.stackMaxPerSide, ddPct: safest.maxDrawdownPct },
  };
  const jsonPath = join(assetsDir, "backtest_5mall_stack_sweep_3y.json");
  writeFileSync(jsonPath, JSON.stringify(out, null, 2));
  console.log(`\n💾 JSON  → ${jsonPath}`);

  const htmlPath = join(assetsDir, "backtest_5mall_stack_sweep_3y_report.html");

/**
 * fetch-eth-sol-5m-3y.ts — Fetch ETHUSDT + SOLUSDT 5m 3y từ Binance Spot.
 * Save vào .cache/binance-{eth,sol}-5m-3y.json. ~315k bars each, ~2 min.
 */
...
async function main() {
  for (const symbol of ["ETHUSDT", "SOLUSDT"]) {
    const outPath = join(__dirname, "..", ".cache", `binance-${symbol.replace("USDT", "").toLowerCase()}-5m-3y.json`);
    ...
    console.log(`  Saved ${outPath} (${(JSON.stringify(candles).length / 1024 / 1024).toFixed(1)} MB)`);
  }
}

/**
 * champion-volscale-floor-test.mjs — test "volScale Floor" proposal: skip entry if volScale < floor.
 * Faithful champion BEST book (BTC4h+ETH4h, TP16, w1.25, eqvol $100k). Compare floors [0=none, 0.4, 0.5, 0.6].
 * volScale = max(0.3, 1 - atrPctile200). vs<floor => high-vol regime entry skipped.
 * Hypothesis: floor HURTS (skips capitulation-bounce entries volScale already sizes down for). Judge Calmar+CAGR+DD.
 */
...
function gen(P, cfg, e200dAt, sleeve, bandMode, floor) {
  const { c, h, l, t, e200, e20, rsi: R, atr: A, adx, pdi, mdi } = P;
  let pos = [], out = [], last = -999, skipped = 0;
  for (let i = 200; i < c.length - cfg.hold - 1; i++) {
    ...
    if (vs < floor) { skipped++; continue; }   // ── volScale FLOOR (proposal under test) ──
    pos.push({ ei: i, epx: price, sl: price - cfg.slAtr * at, tp: price + TP_OVERRIDE * at, ems: t[i], vs });
    last = i;
  }
  return { out, skipped };
}
function simEqvol(trades) {
  ...
  return { finalEq: equity, cagr, maxDD, calmar: maxDD > 0 ? cagr / maxDD : Infinity, n: trades.length };
}
console.log("=== CHAMPION volScale-FLOOR test (BTC4h+ETH4h BEST, eqvol $100k) ===");
console.log("floor = skip entry khi volScale < floor (vol cao). 0=none=LIVE\n");
console.log("floor | n    | skipped | CAGR% | maxDD% | Calmar");
for (const floor of [0.0, 0.4, 0.5, 0.6]) {
  const g4 = gen(P4btc, CHAMPION.btc4h, btcE200d, "BTC4h", "above", floor);
  const gE = gen(P4eth, CHAMPION.eth4h, ethE200d, "ETH4h", "band", floor);
  const all = [...g4.out, ...gE.out].sort((a, b) => a.eTime - b.eTime);
  const r = simEqvol(all);
  console.log(`${floor.toFixed(1).padStart(5)} | ${String(r.n).padStart(4)} | ${String(g4.skipped + gE.skipped).padStart(7)} | ${(r.cagr * 100).toFixed(0).padStart(5)} | ${(r.maxDD * 100).toFixed(1).padStart(6)} | ${r.calmar.toFixed(2).padStart(6)}`);
}

#!/usr/bin/env python3
"""autoloop-r73-rangegate-aug.py — Round 73: RANGE gate + August filter verify
R72 findings:
  - RANGE_gate≥10-14 days → Sh+2.19 (+0.017 ★)
  - August: 0% WR, -2.2% avg → potential skip candidate
  - BTC SL2.78/3.28: 3/3 robust (+0.005)

R73:
  A: RANGE_gate robustness — holdout
  B: RANGE_gate fine sweep + combine with BTC78
  C: August exclusion filter — skip August entirely
  D: August exclusion robustness
  E: Combine RANGE_gate + Aug-skip + BTC78
  F: Other weak months — exclude any month with WR<30% consistently
"""
...
def run_h01(cache, adx_thresh=18, sl_init=3.0, sl_trail=3.5, sl_trans=16,
             ...
             range_gate_days=0, skip_months=None):
    H.CACHE=cache
    ...
    # RANGE consecutive days
    reg_days_sorted=sorted(reg_map.keys()); consecutive=0; prev_reg=None
    range_start_day={}
    for dk in reg_days_sorted:
        r=reg_map[dk]
        consecutive=(consecutive+1) if (r=="RANGE" and prev_reg=="RANGE") else (0 if r=="RANGE" else 0)
        range_start_day[dk]=consecutive
        prev_reg=r
    def get_reg(ts): return reg_map.get(ts//86400000,"RANGE")
    def range_days(ts): return range_start_day.get(ts//86400000,0)
    ...
    def filt(i):
        ...
        if range_gate_days>0 and range_days(bars4h[i]["time"])<range_gate_days: return False
        # calendar filter
        if skip_months:
...
sh_base=pr("R70 BASELINE: no gate no skip",[sB28,sS29,sTB14])[0]
print(); print("="*100); print("=== R73: RANGE gate + August skip verify ===\n")

# A: RANGE gate robustness
print("━"*100); print("A: RANGE_gate=10d robustness — holdout years")
moB_10,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.8,sl_trail=3.3,range_gate_days=10)
sB10=[moB_10.get(m,0.0) for m in cal3]
pr("  gate=10 BTC",[sB10,sS29,sTB14])
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB28_ho=[moB28.get(m,0.0) for m in cal_ho]; sB10_ho=[moB_10.get(m,0.0) for m in cal_ho]
    sS_ho=[moS29.get(m,0.0) for m in cal_ho]; sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_0,_,_,_,_=bk([sB28_ho,sS_ho,sTB_ho],cal=cal_ho)
    sh_10,_,_,_,_=bk([sB10_ho,sS_ho,sTB_ho],cal=cal_ho)
    robust="✓ ROBUST" if sh_10>sh_0 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: gate=0 Sh{sh_0:+.2f} → gate=10 Sh{sh_10:+.2f} delta{sh_10-sh_0:>+.3f} {robust}")

# B: RANGE gate fine sweep + BTC78
print(f"\n{'━'*100}"); print("B: RANGE gate fine sweep with BTC78 (best micro)")
for gate in [0,5,7,10,12,14,16]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,range_gate_days=gate)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC78 gate={gate}",[sB_,sS29,sTB14],extra=f"n={nt}")

# C: August exclusion
print(f"\n{'━'*100}"); print("C: August exclusion (skip Aug trades)")
moB_noaug,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.8,sl_trail=3.3,skip_months={8})
sB_noaug=[moB_noaug.get(m,0.0) for m in cal3]
pr(f"  BTC skip-Aug (gate=0)",[sB_noaug,sS29,sTB14],extra=f"n={nt}")

# Aug robustness
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB28_ho=[moB28.get(m,0.0) for m in cal_ho]; sBna_ho=[moB_noaug.get(m,0.0) for m in cal_ho]
    sS_ho=[moS29.get(m,0.0) for m in cal_ho]; sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_0,_,_,_,_=bk([sB28_ho,sS_ho,sTB_ho],cal=cal_ho)
    sh_na,_,_,_,_=bk([sBna_ho,sS_ho,sTB_ho],cal=cal_ho)
    robust="✓" if sh_na>sh_0 else "✗"
    print(f"  Holdout {yr_exc}: no-skip Sh{sh_0:+.2f} → skip-Aug Sh{sh_na:+.2f} delta{sh_na-sh_0:>+.3f} {robust}")

# D: Aug exclusion + other months (BTC only)
print(f"\n{'━'*100}"); print("D: Calendar filter — skip weak months")
# Aug most consistent bad. Check Aug on full 7y BTC
moB7,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.8,sl_trail=3.3)
...
print(f"  BTC 7y monthly stats:")
for mo_n in range(1,13):
    vals=by_mo7[mo_n]; wr=sum(1 for v in vals if v>0)/len(vals)*100 if vals else 0
    avg=sum(vals)/len(vals)*100 if vals else 0
    tag="★BAD★" if wr<30 else ("GOOD" if wr>60 else "")
    print(f"  Mo{mo_n:>2}: WR{wr:>4.0f}% avg{avg:>+5.1f}% n={len(vals)} {tag}")

# Aug skip + gate combo
print(f"\n{'━'*100}"); print("E: FULL COMBINE — gate + skip + BTC78 + SOL29")
best_combo_sh=sh_base
for gate in [0,10,14]:
    for skip_mo in [None,{8},{8,9}]:
        moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,
                         range_gate_days=gate,skip_months=skip_mo)
        sB_=[moB_.get(m,0.0) for m in cal3]
        lbl=f"BTC78 gate={gate} skip={skip_mo}"
        sh,md=pr(f"  {lbl}",[sB_,sS29,sTB14],extra=f"n={nt}")
        if sh>best_combo_sh: best_combo_sh=sh

# F: SOL with gate/skip
print(f"\n{'━'*100}"); print("F: SOL with gate (does gate help SOL too?)")
moS_10,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,range_gate_days=10)
moS_noaug,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,skip_months={8})
sS10=[moS_10.get(m,0.0) for m in cal3]; sSna=[moS_noaug.get(m,0.0) for m in cal3]
pr("  BTC28 SOL gate=10",[sB28,sS10,sTB14])
pr("  BTC28 SOL skip-Aug",[sB28,sSna,sTB14])
pr("  BTC78 SOL gate=10",[sB78,sS10,sTB14])
pr("  BTC78 gate=10 SOL gate=10",[moB_10 and [moB_10.get(m,0.0) for m in cal3],sS10,sTB14])

print(f"\n{'='*100}")
print(f"R73 SUMMARY — ceiling from Sh{sh_base:+.3f}")
print(f"  Best combo attempt: {best_combo_sh:+.3f}")
print(f"  RANGE gate ROBUST? → check section A")
print(f"  August skip ROBUST? → check section C")

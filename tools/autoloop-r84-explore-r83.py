#!/usr/bin/env python3
"""autoloop-r84-explore-r83.py — Round 84: Explore with R83 config
R83 FINAL: BTC(D18,vm16,SL2.78,Aug-skip) + SOL(vm16,SL2.9) + DX14+T1.2 → Sh+2.38 DD4.6%

R84:
  A: Full 3-holdout verify R83 vs R50
  B: SOL D_LB with vol_ma=16 (D_LB=20 was optimal without vm change, re-check)
  C: BTC SL micro with R83 (SL 2.76-2.80)
  D: SOL SL micro with R83 (SL 2.85-2.92)
  E: NEW: ATR_BREAK_MULT SOL sweep with vm16 config
"""
...
print("Loading R83 config...")
moB,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16)
moS,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16)
...
sS10=[moS10.get(m,0.0) for m in cal3]; sTB14=[turB14.get(m,0.0) for m in cal3]; W=[1,1,1.2]
...
def pr(label, parts, weights=None, cal=None, extra=""):
    ...
    star="★★★★" if sh>=2.42 else ("★★★" if sh>=2.40 else ("★★" if sh>=2.38 else ("★" if delta>0.05 else "")))
    print(f"  {mark} {label:<54} Sh{sh:>+5.2f} DD{md:>4.1f}% flat{fl:>3} TE{te:>+5.2f} d{delta:>+.3f} {star} {extra}")
    return sh,md

sh_base,_=pr("R83 FINAL (BTC_vm16+SOL_vm16)",[sB,sS,sTB14])
print(); print("="*100); print("=== R84: Explore with R83 config ===\n")

# A: Full holdout R83 vs R50
print("━"*100); print("A: R83 vs R50 canonical — full holdout (definitive)")
...
for yr_exc in [2023,2024,2025]:
    ...
    sh_r83,md_r83,_,_,_=bk([sB_ho,sS_ho,sTB14_ho],W,cal=cal_ho)
    delta=sh_r83-sh_can
    print(f"  Holdout {yr_exc}: R50 Sh{sh_can:+.2f} DD{md_can:.1f}% → R83 Sh{sh_r83:+.2f} DD{md_r83:.1f}% delta{delta:>+.3f} {'✓' if delta>0 else '✗'}")

# B: SOL D_LB with vol_ma=16
print(f"\n{'━'*100}"); print("B: SOL D_LB sweep with vol_ma=16")
for dlb in [15,17,18,19,20,22,25]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=dlb,vol_ma=16)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL D_LB={dlb}",[sB,sS_,sTB14],extra=f"n={nt}")

# C: BTC SL micro with R83
print(f"\n{'━'*100}"); print("C: BTC SL micro with R83 config")
for si,st in [(2.76,3.26),(2.77,3.27),(2.78,3.28),(2.79,3.29),(2.80,3.30)]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=si,sl_trail=st,skip_months={8},donchian_lb=18,vol_ma=16)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC SL{si}/{st}",[sB_,sS,sTB14],extra=f"n={nt}")

# D: SOL SL micro with R83 (vm16)
print(f"\n{'━'*100}"); print("D: SOL SL micro with vm16")
for si,st in [(2.85,3.35),(2.87,3.37),(2.88,3.38),(2.90,3.40),(2.91,3.41)]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=si,sl_trail=st,sl_trans=24,donchian_lb=20,vol_ma=16)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL SL{si}/{st}",[sB,sS_,sTB14],extra=f"n={nt}")

# E: SOL ATR_BREAK_MULT with vm16
print(f"\n{'━'*100}"); print("E: SOL ATR_BREAK_MULT with vm16")
for abm in [1.0,1.1,1.2,1.3,1.4,1.5]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=abm)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL ABM={abm}",[sB,sS_,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R84 SUMMARY — Ceiling Sh{sh_base:+.3f}")
print(f"  R83 CONFIRMED BEST CONFIG:")
print(f"    BTC: ADX18, SL2.78/3.28/16, D_LB=18, vol_ma=16, skip Aug")
print(f"    SOL: ADX15, SL2.9/3.4/24, D_LB=20, vol_ma=16")
print(f"    Turtle: DE20/DX14/CUT2.0/BEAR-gate")
print(f"    Weight: BTC=1, SOL=1, Turtle=1.2")
print(f"    Sh+2.38 DD4.6%")

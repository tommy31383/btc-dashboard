#!/usr/bin/env python3
"""autoloop-r69-turtle-dx.py — Round 69: Turtle DX=25 verify + combine
R68 finding: DX=25 → Sh+2.18 (+0.031 ★, TE+1.58), DX=14 → Sh+2.17 (+0.023)
  A: DX fine sweep 14-30 confirm peak
  B: DX=25 robustness — holdout years
  C: DX=25 + DE sweep (might interact)
  D: DX=25 + CUT sweep (wider exit → maybe different optimal CUT?)
  E: DX=14 robustness (backup option)
  F: Full combine DX=25 + R67 config → FINAL BEST?
"""
...
turB15=run_turtle(20,15,2.0); turB25=run_turtle(20,25,2.0)
...
sTB15=[turB15.get(m,0.0) for m in cal3]; sTB25=[turB25.get(m,0.0) for m in cal3]
W=[1,1,1.2]
...
sh_base=pr("R67 BASELINE: DX=15",[sB28,sS29,sTB15])[0]
pr("  DX=25 (R68 find)",[sB28,sS29,sTB25])
print(); print("="*100); print("=== R69: DX=25 verify + combine ===\n")

# A: DX fine sweep
print("━"*100); print("A: Turtle DX fine sweep 14-30 — confirm peak")
best_dx=15; best_dx_sh=sh_base; best_tur=turB15
for DX in range(14,31):
    tur_=run_turtle(20,DX,2.0); sTB_=[tur_.get(m,0.0) for m in cal3]
    sh,md=pr(f"  DX={DX}",[sB28,sS29,sTB_])
    if sh>best_dx_sh: best_dx_sh=sh; best_dx=DX; best_tur=tur_
print(f"\n  → Best DX={best_dx} Sh{best_dx_sh:+.3f}")

# B: DX=25 robustness
print(f"\n{'━'*100}"); print("B: DX=25 vs DX=15 robustness — holdout years")
for yr_exc in [2023,2024,2025]:
    ...
    sTB15_ho=[turB15.get(m,0.0) for m in cal_ho]; sTB25_ho=[turB25.get(m,0.0) for m in cal_ho]
    sh_15,_,_,_,_=bk([sB_ho,sS_ho,sTB15_ho],cal=cal_ho)
    sh_25,_,_,_,_=bk([sB_ho,sS_ho,sTB25_ho],cal=cal_ho)
    robust="✓ ROBUST" if sh_25>sh_15 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: DX=15 Sh{sh_15:+.2f} → DX=25 Sh{sh_25:+.2f} delta{sh_25-sh_15:>+.3f} {robust}")

# C: DX=best + DE sweep
print(f"\n{'━'*100}"); print(f"C: DE sweep with DX={best_dx}")
for DE in [16,18,20,22,24]:
    tur_=run_turtle(DE,best_dx,2.0); sTB_=[tur_.get(m,0.0) for m in cal3]
    pr(f"  DE={DE} DX={best_dx}",[sB28,sS29,sTB_])

# D: DX=25 + CUT sweep
print(f"\n{'━'*100}"); print("D: DX=25 + CUT sweep")
for CUT in [1.5,1.8,2.0,2.2,2.5]:
    tur_=run_turtle(20,25,CUT); sTB_=[tur_.get(m,0.0) for m in cal3]
    pr(f"  DX=25 CUT={CUT}",[sB28,sS29,sTB_])

# E: DX=14 robustness
print(f"\n{'━'*100}"); print("E: DX=14 robustness — holdout years")
...
for yr_exc in [2023,2024,2025]:
    ...
    sh_15,_,_,_,_=bk([sB_ho,sS_ho,sTB15_ho],cal=cal_ho)
    sh_14,_,_,_,_=bk([sB_ho,sS_ho,sTB14_ho],cal=cal_ho)
    robust="✓" if sh_14>sh_15 else "✗"
    print(f"  Holdout {yr_exc}: DX=15 Sh{sh_15:+.2f} → DX=14 Sh{sh_14:+.2f} delta{sh_14-sh_15:>+.3f} {robust}")

# F: Final comparison
print(f"\n{'━'*100}"); print("F: FINAL CONFIG candidates")
sTBbest=[best_tur.get(m,0.0) for m in cal3] if best_dx!=15 else sTB15
for lbl,sTB_,w in [
    ("R67: DX=15 T1.2",sTB15,W),
    ("R69: DX=25 T1.2",sTB25,W),
    (f"R69: DX={best_dx} T1.2",sTBbest,W),
    ("R69: DX=25 T1.5",sTB25,[1,1,1.5]),
    ("R69: DX=25 equal w",sTB25,[1,1,1]),
]:
    pr(f"  {lbl}",[sB28,sS29,sTB_],weights=w)

print(f"\n{'='*100}")
print(f"R69 SUMMARY")
print(f"  R67 baseline: Sh{sh_base:+.3f}")
print(f"  Best DX found: {best_dx} → Sh{best_dx_sh:+.3f}")

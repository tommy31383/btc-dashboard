/**
 * backtest-eth-tweaks-3y.ts — Test ETH-specific tweaks vs v0.4.46 baseline.
 *
 * Variants:
 *   BASELINE: rule v0.4.46 default
 *   E1: drop S12 (WR 42.9% on ETH, -$98 loss)
 *   E2: S13 cooldown 4h → 2h (S13 best ETH setup, WR 63%, +$359)
 *   E3: trailing ATR×2.5 post-48h (ETH avg loss bigger, tighten)
 *   E1+E2 combo
 *   E1+E3 combo
 *   E_ALL: E1+E2+E3
 */
...
interface VariantCfg {
  ...
  trailingSL: number;  // post-48h trailing mult
}

const VARIANTS: VariantCfg[] = [
  { name: "ETH_BASELINE", dropS12: false, s13CooldownHrs: 4, trailingSL: 3.0 },
  { name: "E1_dropS12",   dropS12: true,  s13CooldownHrs: 4, trailingSL: 3.0 },
  { name: "E2_S13cd2h",   dropS12: false, s13CooldownHrs: 2, trailingSL: 3.0 },
  { name: "E3_trail25",   dropS12: false, s13CooldownHrs: 4, trailingSL: 2.5 },
  { name: "E1_E2",        dropS12: true,  s13CooldownHrs: 2, trailingSL: 3.0 },
  { name: "E1_E3",        dropS12: true,  s13CooldownHrs: 4, trailingSL: 2.5 },
  { name: "E_ALL",        dropS12: true,  s13CooldownHrs: 2, trailingSL: 2.5 },
];

function run(c5: Candle[], cfg: VariantCfg): any {
  ...
  const byYear: Record<string, { entries: number; pnl: number; wins: number }> = {};
  ...
  for (let i = startIdx; i < filtered.length; i++) {
    ...
    for (const t of trendTrades) {
      ...
      const slMult = heldMs < SL_TRANSITION_MS ? SL_INITIAL : cfg.trailingSL;
      if (t.side === "LONG") {
        if (mark > t.hwm) { t.hwm = mark; t.slPx = t.hwm - t.atrEntry * slMult; }
        else if (heldMs >= SL_TRANSITION_MS) { const tgt = t.hwm - t.atrEntry * cfg.trailingSL; if (tgt > t.slPx) t.slPx = tgt; }
      } else {
        if (mark < t.lwm) { t.lwm = mark; t.slPx = t.lwm + t.atrEntry * slMult; }
        else if (heldMs >= SL_TRANSITION_MS) { const tgt = t.lwm + t.atrEntry * cfg.trailingSL; if (tgt < t.slPx) t.slPx = tgt; }
      }
      let exit = false;
      ...
      if (exit) {
        ...
        byYear[y] = byYear[y] ?? { entries: 0, pnl: 0, wins: 0 };
    ...
    const trendEnter = (kind: string, side: "LONG" | "SHORT", notMult: number, lastTsRef: { v: number }, cdMs: number) => {
      ...
      const y = new Date(ts).toISOString().slice(0, 4);
      byYear[y] = byYear[y] ?? { entries: 0, pnl: 0, wins: 0 }; byYear[y].entries++;
    };
  ...
  return { entries, closes, wr: +wr.toFixed(1), roi: +roi.toFixed(2), maxDD: +maxDD.toFixed(2), ra: +ra.toFixed(2), byYear, bySetup, stab: `${stab}/${total}` };
}

function main() {
  console.log("[bt-eth-tweaks] Loading ETH 3y...");
  const c5eth = loadCache("binance-eth-5m-3y.json");
  console.log(`  ${c5eth.length} bars\n`);

  const results: any[] = [];
  for (const cfg of VARIANTS) {
    const r = run(c5eth, cfg);
    results.push({ scenario: cfg.name, ...r });
    console.log(`${cfg.name.padEnd(16)} | RA ${r.ra} | ROI ${r.roi}% | DD ${r.maxDD}% | Entries ${r.entries} | WR ${r.wr}% | Stab ${r.stab}`);
  }
  ...
  for (const r of results) {
    const arrow = r.scenario === "ETH_BASELINE" ? " ⭐BASELINE" : "";
    console.log(`${r.scenario.padEnd(15)} | ${String(r.ra).padStart(4)} | ${String(r.roi).padStart(5)} | ${String(r.maxDD).padStart(4)} | ${String(r.entries).padStart(7)} | ${String(r.wr).padStart(5)} | ${r.stab.padStart(4)}${arrow}`);
  }

  console.log("\n=== PER-YEAR ===");
  for (const r of results) {
    const years = Object.entries(r.byYear).sort().map(([y, d]: any) => `${y}=${d.pnl >= 0 ? '+' : ''}${d.pnl.toFixed(0)}`).join(' ');
    console.log(`${r.scenario.padEnd(16)} | ${years}`);
  }

  console.log("\n=== PER-SETUP ===");
  for (const r of results) {
    const setups = Object.entries(r.bySetup).sort().map(([k, d]: any) => {
      const wr2 = d.entries > 0 ? d.wins / d.entries * 100 : 0;
      return `${k}=n${d.entries}/wr${wr2.toFixed(0)}/$${d.pnl.toFixed(0)}`;
    }).join(' ');
    console.log(`${r.scenario.padEnd(16)} | ${setups}`);
  }

  writeFileSync(join(__dirname, "..", "assets", "backtest_eth_tweaks_3y.json"), JSON.stringify(results, null, 2));
  console.log("\nWritten assets/backtest_eth_tweaks_3y.json");
}

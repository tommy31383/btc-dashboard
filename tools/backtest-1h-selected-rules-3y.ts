...
function simulate(candles: Candle[], entryIdx: number, side: Side): Outcome {
...
async function buildRecords(side: Side) {
  ...
  const records: { time: number; features: Record<string, string>; outcome: Outcome }[] = [];
  const startIdx = 50;
  const endIdx = entry.length - MAX_HOLD - 1;
  for (let i = startIdx; i < endIdx; i++) {
    if (entry[i].time < START_TIME) continue;
    const outcome = simulate(entry, i, side);
    const c = entry[i];
...
function loadCandidates(side: Side) {
  const scanPath = `E:/AI/BTC/btc-dashboard/assets/scan_features_${side}_tp3sl2.json`;
  const scan = JSON.parse(readFileSync(scanPath, "utf8"));
  const tf = scan.tfs["1h"];
  const pairs = Object.entries<any>(tf.pairStats)
    .filter(([, s]) => s.n >= MIN_N && s.wr >= MIN_WR)
    .map(([rule, s]) => ({ kind: "pair", rule, scanN: s.n, scanWR: s.wr }));
  const triples = Object.entries<any>(tf.tripleStats)
    .filter(([, s]) => s.n >= MIN_N && s.wr >= MIN_WR)
    .map(([rule, s]) => ({ kind: "triple", rule, scanN: s.n, scanWR: s.wr }));
  return [...pairs, ...triples];
}

async function runForSide(side: Side) {
  console.log(`Running 3Y backtest for ${side} scanned 1H rules...`);
  const candidates = loadCandidates(side);
  const baseline = side === "LONG" ? 39.8 : 41.3;
  ...
  const results = candidates.map((candidate) => {
    ...
    return {
      ...candidate,
      ...summary,
      edgeVsBaseline: Number((summary.winRate - baseline).toFixed(2)),
    };
  });
  return {
    side,
    ...
    topByNet: [...results].sort((a, b) => b.netRawPct - a.netRawPct).slice(0, 25),
    topByWinRate: [...results].sort((a, b) => b.winRate - a.winRate).slice(0, 25),
    still60: [...results].filter((r) => r.winRate >= 60).sort((a, b) => b.netRawPct - a.netRawPct).slice(0, 25),
  };
}
...
async function main() {
  ...
  const out = {
    ...
    period: "3 years",
    params: { tp: TP_PCT, sl: SL_PCT, hold: MAX_HOLD, minWr: MIN_WR, minN: MIN_N },
    long: longRes,
    short: shortRes,
  };
  const outPath = "E:/AI/BTC/btc-dashboard/assets/selected_1h_rules_3y_backtest.json";
  writeFileSync(outPath, JSON.stringify(out, null, 2));
  console.log(JSON.stringify({
    longTop: longRes.topByNet.slice(0, 5),
    shortTop: shortRes.topByNet.slice(0, 5),
    outPath,
  }, null, 2));
}

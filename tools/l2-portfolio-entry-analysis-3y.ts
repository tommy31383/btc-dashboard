/**
 * l2-portfolio-entry-analysis-3y.ts — 2 parts:
 *   A) Portfolio 60/40 backtest với BTC L2 (skip h=8) + ETH E1 (drop S12)
 *   B) Per-trade entry conditions logged → analyze losses pattern
 *      (ADX value, ATR%ile rank, regime, EMA1h distance, vol ratio, hour, DOW)
 */
...
interface TrendTrade {
  ...
  ec: { adx: number; atrPctile: number; regime: Regime; emaDistPct: number; volRatio: number; hour: number };
}
interface ClosedTrade {
  symbol: string; kind: string; side: "LONG" | "SHORT";
  entryTs: string; exitTs: string; holdH: number; net: number; pnl: number;
  ec: { adx: number; atrPctile: number; regime: Regime; emaDistPct: number; volRatio: number; hour: number };
}

function run(symbol: string, c5: Candle[], skipH8: boolean, dropS12: boolean): { closedTrades: ClosedTrade[]; meta: any; monthlyPnL: Record<string, number> } {
  ...
  for (let i = startIdx; i < filtered.length; i++) {
    ...
    for (const t of trendTrades) {
      ...
      if (exit) {
        ...
        closedTrades.push({
          symbol, kind: t.kind, side: t.side,
          entryTs: new Date(t.entryTs).toISOString(), exitTs: new Date(ts).toISOString(),
          holdH: +((ts - t.entryTs) / 3_600_000).toFixed(1),
          net: +net.toFixed(2), pnl: +pnl.toFixed(2), ec: t.ec,
        });
    ...
    if (idx1hc >= 200) {
      ...
      if (e === null) { emaLong = false; emaShort = false; }
        ...
        emaDistPct = (c1h[idx1hc].close - e) / e * 100;
        if (c1h[idx1hc].close < e) emaLong = false;
        if (c1h[idx1hc].close > e) emaShort = false;
      }
    ...
    if (idx4hc >= 90) {
      ...
      if (cur4hATR === null) atrGatePass = false;
        ...
        atrPctile = arr.findIndex(v => v >= curPct) / arr.length;
    ...
    if (ma !== null && ma > 0) volRatio = vol / ma;
    const vol4hPass = volRatio >= VOL_MULT;
    ...
    if (skipH8 && utcHour === 8) { filterLong = false; filterShort = false; }
    ...
    const trendEnter = (kind: string, side: "LONG" | "SHORT", notMult: number, lastTsRef: { v: number }, cdMs: number) => {
      ...
      const ec = { adx: +(adxVal4h ?? 0).toFixed(1), atrPctile: +atrPctile.toFixed(2), regime, emaDistPct: +emaDistPct.toFixed(2), volRatio: +volRatio.toFixed(2), hour: utcHour };
      trendTrades.push({ id: `${kind}_${side}_${ts}`, kind, side, entryPx: mark, qty, notional, hwm: mark, lwm: mark, slPx, atrEntry: atrVal4h, entryTs: ts, ec });
      lastTsRef.v = ts;
    };
  ...
  return { closedTrades, meta: { roi: +roi.toFixed(2), dd: +maxDD.toFixed(2), ra: +ra.toFixed(2), n: closedTrades.length }, monthlyPnL };
}
...
function main() {
  console.log("[A+B] Backtesting BTC L2 + ETH E1...");
  const btc = run("BTC", loadCache("binance-5m-7y.json"), true, false);   // L2 skip h=8
  const eth = run("ETH", loadCache("binance-eth-5m-3y.json"), false, true);  // E1 drop S12
  console.log(`  BTC L2: n=${btc.meta.n}, ROI ${btc.meta.roi}%, RA ${btc.meta.ra}`);
  console.log(`  ETH E1: n=${eth.meta.n}, ROI ${eth.meta.roi}%, RA ${eth.meta.ra}`);

  // A) Portfolio 60/40 with new variants
  console.log("\n=== A) PORTFOLIO 60/40 — BTC L2 + ETH E1 ===");
  const allMonths = Array.from(new Set([...Object.keys(btc.monthlyPnL), ...Object.keys(eth.monthlyPnL)])).sort();
  const btcM = allMonths.map(m => btc.monthlyPnL[m] ?? 0);
  const ethM = allMonths.map(m => eth.monthlyPnL[m] ?? 0);
  const portM = btcM.map((b, i) => 0.6 * b + 0.4 * ethM[i]);
  const portStats = statsFromMonthly(portM, INITIAL_CAPITAL);
  console.log(`  Portfolio: ROI ${portStats.roi}% | DD ${portStats.dd}% | RA ${portStats.ra} | Sharpe ${portStats.sharpe}`);
  const yearPnL: Record<string, number> = {};
  for (let i = 0; i < allMonths.length; i++) { const y = allMonths[i].slice(0, 4); yearPnL[y] = (yearPnL[y] ?? 0) + portM[i]; }
  console.log(`  Per-year:`);
  for (const [y, p] of Object.entries(yearPnL).sort()) console.log(`    ${y}: ${p >= 0 ? '+' : ''}$${p.toFixed(0)}`);

  // B) Entry conditions analysis for BTC losses
  console.log("\n=== B) BTC LOSSES — ENTRY CONDITIONS ANALYSIS ===");
  const btcLosses = btc.closedTrades.filter(t => t.pnl <= 0);
  const btcWins = btc.closedTrades.filter(t => t.pnl > 0);
  console.log(`BTC L2: Total ${btc.closedTrades.length} (W ${btcWins.length} / L ${btcLosses.length})`);

  // Group by ADX bucket
  console.log("\nADX value bucket (entry):");
  const adxBuckets = [[20, 25], [25, 30], [30, 35], [35, 100]];
  for (const [lo, hi] of adxBuckets) {
    const sub = btc.closedTrades.filter(t => t.ec.adx >= lo && t.ec.adx < hi);
    if (!sub.length) continue;
    const w = sub.filter(t => t.pnl > 0).length;
    const pnl = sub.reduce((s, t) => s + t.net, 0);
    console.log(`  ADX ${lo}-${hi}: n=${sub.length}, WR ${(w/sub.length*100).toFixed(1)}%, PnL $${pnl.toFixed(0)}`);
  }

  // Group by ATR percentile bucket
  console.log("\nATR percentile (entry):");
  const atrBuckets = [[0.30, 0.50], [0.50, 0.70], [0.70, 0.85], [0.85, 1.0]];
  for (const [lo, hi] of atrBuckets) {
    const sub = btc.closedTrades.filter(t => t.ec.atrPctile >= lo && t.ec.atrPctile < hi);
    if (!sub.length) continue;
    const w = sub.filter(t => t.pnl > 0).length;
    const pnl = sub.reduce((s, t) => s + t.net, 0);
    console.log(`  ATR%ile ${(lo*100).toFixed(0)}-${(hi*100).toFixed(0)}: n=${sub.length}, WR ${(w/sub.length*100).toFixed(1)}%, PnL $${pnl.toFixed(0)}`);
  }

  // Group by regime
  console.log("\nRegime at entry:");
  for (const reg of ["BULL", "RANGE", "BEAR"]) {
    const sub = btc.closedTrades.filter(t => t.ec.regime === reg);
    if (!sub.length) continue;
    const w = sub.filter(t => t.pnl > 0).length;
    const pnl = sub.reduce((s, t) => s + t.net, 0);
    console.log(`  ${reg.padEnd(6)}: n=${sub.length}, WR ${(w/sub.length*100).toFixed(1)}%, PnL $${pnl.toFixed(0)}`);
  }

  // EMA distance bucket
  console.log("\nEMA200 1h distance % (entry, signed):");
  const emaBuckets = [[-99, -1.5], [-1.5, -0.5], [-0.5, 0.5], [0.5, 1.5], [1.5, 99]];
  for (const [lo, hi] of emaBuckets) {
    const sub = btc.closedTrades.filter(t => t.ec.emaDistPct >= lo && t.ec.emaDistPct < hi);
    if (!sub.length) continue;
    const w = sub.filter(t => t.pnl > 0).length;
    const pnl = sub.reduce((s, t) => s + t.net, 0);
    console.log(`  emaDist ${lo}-${hi}%: n=${sub.length}, WR ${(w/sub.length*100).toFixed(1)}%, PnL $${pnl.toFixed(0)}`);
  }

  // Vol ratio bucket
  console.log("\nVol/MA10 ratio (entry):");
  const volBuckets = [[1.2, 1.5], [1.5, 2.0], [2.0, 3.0], [3.0, 99]];
  for (const [lo, hi] of volBuckets) {
    const sub = btc.closedTrades.filter(t => t.ec.volRatio >= lo && t.ec.volRatio < hi);
    if (!sub.length) continue;
    const w = sub.filter(t => t.pnl > 0).length;
    const pnl = sub.reduce((s, t) => s + t.net, 0);
    console.log(`  vol ${lo}-${hi}×: n=${sub.length}, WR ${(w/sub.length*100).toFixed(1)}%, PnL $${pnl.toFixed(0)}`);
  }

  writeFileSync(join(__dirname, "..", "assets", "l2_portfolio_entry_analysis_3y.json"), JSON.stringify({
    a_portfolio_60_40: { stats: portStats, yearPnL, monthly: { btc: btcM, eth: ethM, port: portM, months: allMonths } },
    b_btc_trades: btc.closedTrades, b_eth_trades: eth.closedTrades,
    meta: { btc: btc.meta, eth: eth.meta },
  }, null, 2));
  console.log("\nWritten assets/l2_portfolio_entry_analysis_3y.json");
}

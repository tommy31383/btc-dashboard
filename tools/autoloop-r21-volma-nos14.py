#!/usr/bin/env python3
"""autoloop-r21-volma-nos14.py — Round 21: VOL_MA + no-S14 with winner R20"""
...
def run_h01(cache, adx_thresh=18, sl_init=3.0, sl_trail=3.5, sl_trans=16,
            adx_period=12, atr_break_mult=1.3, vol_mult=1.4, vol_ma=10, skip_s14=False):
    H.CACHE=cache
    orig_adx=H.ADX_THRESH; orig_si=H.SL_INIT; orig_st=H.SL_TRAIL; orig_tr=H.SL_TRANS
    orig_p=H.ADX_P; orig_abm=H.ATR_BREAK_MULT; orig_vm=H.VOL_MULT; orig_vma=H.VOL_MA
    ...
    H.ADX_P=orig_p; H.ATR_BREAK_MULT=orig_abm; H.VOL_MULT=orig_vm; H.VOL_MA=orig_vma
    return mo
...
def r7(adx_t=18,si=3.0,st=3.5,tr=16,p=12,abm=1.3,vm=1.4,vma=10,ns14=False):
    mo=run_h01(f"{CC}/binance-5m-7y.json",adx_t,si,st,tr,p,abm,vm,vma,ns14)
...
print("="*90)
print("=== Round 21: VOL_MA sweep + no-S14 with Winner-R20 ===")
print("=== Winner-R20: ADX12+ABM1.3+VM1.4+BTC18+SOL15+SL(3/3.5/16) Sh+1.91 ===\n")

# ─── R21A: no-S14 with winner R20 ───
print("━"*90)
print("R21A: no-S14 (S12+S13 only) with Winner-R20")
for ns14,label in [(False,"Winner-R20 full"),(True,"Winner-R20 no-S14")]:
    mb=run_h01(f"{CC}/binance-5m-7y.json",18,3.0,3.5,16,12,1.3,1.4,10,ns14)
    ms=run_h01(f"{CC}/binance-sol-5m-3y.json",15,3.0,3.5,16,12,1.3,1.4,10,ns14)
    sb_=[mb.get(m,0.0) for m in cal]; ss_=[ms.get(m,0.0) for m in cal]
    sh_,md_,fl_,sh_nt_,py_=bk(sb_,ss_)
    sh7,md7,py7=r7(ns14=ns14)
    print(f"  {label:<30} Sh{sh_:>+5.2f} DD{md_:>4.1f}% flat{fl_:>3}/35 no-top{sh_nt_:>+5.2f} | 7y:{sh7:>+5.2f} DD{md7:.0f}% | {py_}")

# ─── R21B: VOL_MA sweep ───
print("\n━"*90)
print("R21B: VOL_MA sweep (volume look-back, default=10)")
print(f"  {'VOL_MA':>7} | {'Sh':>6} {'DD':>5} {'flat':>5} | {'7y-Sh':>6} {'7y-DD':>6} | per-year")
print("  "+"-"*70)
best_sh=1.91; best_vma=None
for vma in [5,7,8,10,12,15,20]:
    mb=run_h01(f"{CC}/binance-5m-7y.json",18,3.0,3.5,16,12,1.3,1.4,vma)
    ms=run_h01(f"{CC}/binance-sol-5m-3y.json",15,3.0,3.5,16,12,1.3,1.4,vma)
    sb_=[mb.get(m,0.0) for m in cal]; ss_=[ms.get(m,0.0) for m in cal]
    sh_,md_,fl_,sh_nt_,py_=bk(sb_,ss_)
    sh7,md7,_=r7(vma=vma)
    ok7="✅" if sh7>=0.95 and md7<=30 else "⚠️"
    if sh_>best_sh+0.01 and ok7=="✅": best_sh=sh_; best_vma=vma
    mark="◀ default" if vma==10 else ("★" if sh_>1.92 and ok7=="✅" else "")
    print(f"  {vma:>7} | {sh_:>+6.2f} {md_:>4.1f}% {fl_:>3}/35 | {sh7:>+6.2f} {md7:>5.0f}% {ok7:>4} | {py_} {mark}")
if best_vma: print(f"  ★ BEST VOL_MA: {best_vma} → Sh{best_sh:+.2f}")
else: print(f"  → VOL_MA=10 is optimal for Winner-R20")

# ─── R21C: ATR_PCT_PCTL sweep ───
print("\n━"*90)
print("R21C: ATR_PCT_PCTL sweep (vol-adjusted percentile filter, default=50th)")
for pct in [0.25,0.35,0.40,0.45,0.50,0.55,0.60]:
    orig=H.ATR_PCT_PCTL; H.ATR_PCT_PCTL=pct
    mb=run_h01(f"{CC}/binance-5m-7y.json",18,3.0,3.5,16,12,1.3,1.4)
    ms=run_h01(f"{CC}/binance-sol-5m-3y.json",15,3.0,3.5,16,12,1.3,1.4)
    sb_=[mb.get(m,0.0) for m in cal]; ss_=[ms.get(m,0.0) for m in cal]
    sh_,md_,fl_,sh_nt_,py_=bk(sb_,ss_)
    H.ATR_PCT_PCTL=orig
    mark="◀ default" if pct==0.50 else ("★" if sh_>1.92 else "")
    print(f"  ATR_PCTL {pct:.2f}: Sh{sh_:>+5.2f} DD{md_:>4.1f}% flat{fl_:>3}/35 | {py_} {mark}")

# ─── Summary ───
print("\n"+"="*90)
print("ROUND 21 SUMMARY")
print("  R21 tests: no-S14 + VOL_MA sweep + ATR_PCT_PCTL sweep")
print("  If all ≤1.92 Sh with 7y-robust → Winner-R20 Sh+1.91 is true ceiling")

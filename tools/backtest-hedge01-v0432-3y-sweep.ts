/**
 * backtest-hedge01-v0432-3y-sweep.ts — sweep MAX_QTY scenarios.
 *
 * Sweep MAX_QTY_PER_SIDE ∈ {0.02, 0.05, 0.1, 0.5, 5.0} để tách EDGE của rule
 * khỏi sized-for-200$ constraint. Capital $100k cố định.
 */
...
function runBacktest(c5: Candle[], c15: Candle[], c1h: Candle[], c1d: Candle[], c1w: Candle[], ind: Indicators, maxQty: number): any {
  ...
  for (let i = 0; i < c5.length; i++) {
    ...
    if (!longCD) {
      ...
      if (lS >= 9) {
        ...
        if (idx5c >= 200) {
          ...
          const m200 = ind.ma200_5m[idx5c]; const m50 = ind.ma50_5m[idx5c];
          let deep = false;
          if (m200 !== null && (last5px - m200) / m200 * 100 < -10) deep = true;
          else if (m50 !== null && (last5px - m50) / m50 * 100 < -5) deep = true;
          else if (idx5c >= 60) {
      ...
      if (qty > 0 && longNet.qty + qty <= maxQty) {
    ...
    if (!s11CD && c2Fire && f4Pass && allowMomLong) {
      ...
      if (scaledQty >= MIN_BINANCE_QTY && longNet.qty + scaledQty <= maxQty) {
    ...
    if (sS >= 9 && !shortCD && allowShortSide) {
      ...
      if (weeklyDown) {
        ...
        if (qty > 0 && qty < MIN_BINANCE_QTY) qty = MIN_BINANCE_QTY;
        if (qty > 0 && shortNet.qty + qty <= maxQty) {
          const fee = qty * mark * FEE_PER_SIDE_PCT / 100;
          wallet -= fee; totalFees += fee;
  ...
  const riskAdj = maxDD > 0 ? roi / maxDD : 0;

  return {
    maxQty, wallet: Math.round(wallet), hwm: Math.round(hwm), lowestWallet: Math.round(lowestWallet),
    ...
    setupCounts, regimeCounts, byYear,
  };
}
...
function main() {
  ...
  const scenarios = [0.02, 0.05, 0.1, 0.5, 5.0];
  const results: any[] = [];
  for (const mq of scenarios) {
    console.log(`\n[sweep] Running MAX_QTY = ${mq}...`);
    const r = runBacktest(c5, c15, c1h, c1d, c1w, ind, mq);
    results.push(r);
    console.log(`  ROI ${r.roi}% / DD ${r.maxDD}% / RiskAdj ${r.riskAdj} / ${r.entries}E ${r.closes}C WR ${r.wr}% Exp $${r.expectancy}/trade`);
  }

  console.log("\n=== SWEEP COMPARISON (capital $100k, 3y) ===");
  console.log("MAX_QTY  | ROI%      | DD%    | RiskAdj | Entries | Closes | WR%   | AvgWin   | AvgLoss   | Exp/trade");
  console.log("-".repeat(115));
  for (const r of results) {
    console.log(`${String(r.maxQty).padEnd(8)} | ${String(r.roi).padStart(9)} | ${String(r.maxDD).padStart(6)} | ${String(r.riskAdj).padStart(7)} | ${String(r.entries).padStart(7)} | ${String(r.closes).padStart(6)} | ${String(r.wr).padStart(5)} | ${String(r.avgWin).padStart(8)} | ${String(r.avgLoss).padStart(9)} | ${String(r.expectancy).padStart(8)}`);
  }

  writeFileSync(join(__dirname, "..", "assets", "backtest_h01_v0432_3y_sweep.json"), JSON.stringify(results, null, 2));
  console.log(`\nWritten assets/backtest_h01_v0432_3y_sweep.json`);
}

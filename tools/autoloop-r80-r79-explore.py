#!/usr/bin/env python3
"""autoloop-r80-r79-explore.py — Round 80: Explore with R79 config
R79 FINAL: BTC(ADX18,SL2.78/3.28,D_LB=18,Aug-skip) + SOL(ADX15,SL2.9/3.4/24) + Turtle(DX14,T1.2)
Sh+2.31 DD5.3% — 3/3 holdout robust + 7y confirmed

R80: Try remaining unexplored directions with this new baseline
  A: ATR_BREAK_MULT with D_LB=18+Aug-skip (previously 1.3 optimal, check with new config)
  B: vol_ma with new config
  C: ADX thresh sweep with D_LB=18+Aug-skip
  D: SOL D_LB sweep with new BTC config
  E: SL fine-tune around 2.78 with D_LB=18+Aug
"""
...
print("Loading R79 config...")
moB,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18)
...
sB=[moB.get(m,0.0) for m in cal3]; sS=[moS.get(m,0.0) for m in cal3]; sTB14=[turB14.get(m,0.0) for m in cal3]
W=[1,1,1.2]
...
sh_base,_=pr("R79 BASELINE",[sB,sS,sTB14])
print(); print("="*100); print("=== R80: Explore with R79 config ===\n")

# A: ATR_BREAK_MULT sweep
print("━"*100); print("A: ATR_BREAK_MULT sweep with D_LB=18+Aug-skip")
for abm in [1.0,1.1,1.2,1.3,1.4,1.5]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,atr_break_mult=abm)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  ABM={abm}",[sB_,sS,sTB14],extra=f"n={nt}")

# B: vol_ma sweep
print(f"\n{'━'*100}"); print("B: vol_ma sweep with D_LB=18+Aug-skip")
for vm in [5,7,10,12,14,16]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=vm)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  vol_ma={vm}",[sB_,sS,sTB14],extra=f"n={nt}")

# C: ADX thresh
print(f"\n{'━'*100}"); print("C: ADX thresh with D_LB=18+Aug-skip")
for adxt in [16,17,18,19,20]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",adxt,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  ADX={adxt}",[sB_,sS,sTB14],extra=f"n={nt}")

# D: SOL D_LB sweep
print(f"\n{'━'*100}"); print("D: SOL D_LB sweep with new BTC config")
for dlb in [15,16,17,18,19,20,22]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=dlb)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL D_LB={dlb}",[sB,sS_,sTB14],extra=f"SOL_n={nt}")

# E: BTC SL micro with D_LB=18
print(f"\n{'━'*100}"); print("E: BTC SL micro-sweep with D_LB=18+Aug")
for si,st in [(2.75,3.25),(2.77,3.27),(2.78,3.28),(2.79,3.29),(2.80,3.30),(2.82,3.32)]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=si,sl_trail=st,skip_months={8},donchian_lb=18)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  SL{si}/{st}",[sB_,sS,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R80 SUMMARY — Ceiling: {sh_base:+.3f}")
print(f"  R79 CONFIRMED FINAL CONFIG:")
print(f"    BTC: ADX18, SL2.78/3.28/16, D_LB=18, RANGE-only, skip August")
print(f"    SOL: ADX15, SL2.9/3.4/24, D_LB=20, RANGE-only")
print(f"    Turtle: DE20/DX14/CUT2.0/BEAR-gate")
print(f"    Weight: BTC=1, SOL=1, Turtle=1.2")
print(f"    Sh+2.31 DD5.3% (vs R50 +2.05: +0.26)")

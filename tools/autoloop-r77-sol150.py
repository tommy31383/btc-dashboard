#!/usr/bin/env python3
"""autoloop-r77-sol150.py — Round 77: SOL EMA150 verify + final ceiling
R76 finds:
  EMA1h=150 BTC: 2/3 robust (2025 flat -0.004)
  BTC150+SOL150 → Sh+2.33 (+0.068 ★★) — higher!
  EMA120-160 all equivalent at Sh+2.31

R77:
  A: SOL EMA150 robustness — holdout
  B: SOL EMA fine sweep 100-200
  C: BTC150+SOL150 full combo robustness
  D: NEW: 7y BTC verify EMA150 vs 200 (is it structural on 7y?)
  E: FINAL CEILING TABLE R50-R77
"""
...
moB150,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=150)
moB200,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=200)
moS200,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,ema_1h=200)
moS150,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,ema_1h=150)
...
sB150=[moB150.get(m,0.0) for m in cal3]; sB200=[moB200.get(m,0.0) for m in cal3]
sS200=[moS200.get(m,0.0) for m in cal3]; sS150=[moS150.get(m,0.0) for m in cal3]
sTB14=[turB14.get(m,0.0) for m in cal3]; W=[1,1,1.2]
...
sh_base,_=pr("R74 BASELINE (BTC200+SOL200)",[sB200,sS200,sTB14])
pr("  R76 BTC150+SOL200",[sB150,sS200,sTB14])
pr("  R76 BTC150+SOL150 (NEW)",[sB150,sS150,sTB14])
print(); print("="*100); print("=== R77: SOL EMA150 verify ===\n")

# A: SOL EMA150 robustness
print("━"*100); print("A: BTC150+SOL150 vs BTC150+SOL200 — holdout years")
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB150_ho=[moB150.get(m,0.0) for m in cal_ho]
    sS200_ho=[moS200.get(m,0.0) for m in cal_ho]; sS150_ho=[moS150.get(m,0.0) for m in cal_ho]
    sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_s200,_,_,_,_=bk([sB150_ho,sS200_ho,sTB_ho],cal=cal_ho)
    sh_s150,_,_,_,_=bk([sB150_ho,sS150_ho,sTB_ho],cal=cal_ho)
    robust="✓ ROBUST" if sh_s150>sh_s200 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: SOL200 Sh{sh_s200:+.2f} → SOL150 Sh{sh_s150:+.2f} delta{sh_s150-sh_s200:>+.3f} {robust}")

# B: SOL EMA sweep
print(f"\n{'━'*100}"); print("B: SOL EMA fine sweep")
best_sol_ema=200; best_sol_sh=sh_base
for ema_s in [100,120,140,150,160,180,200]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,ema_1h=ema_s)
    sS_=[moS_.get(m,0.0) for m in cal3]
    sh,md=pr(f"  BTC150+SOL_EMA{ema_s}",[sB150,sS_,sTB14],extra=f"n={nt}")
    if sh>best_sol_sh: best_sol_sh=sh; best_sol_ema=ema_s

# C: Full holdout for BTC150+SOL150+Aug-skip
print(f"\n{'━'*100}"); print("C: Full holdout — BTC150+SOL150 vs R74 baseline")
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB_base_ho=[moB200.get(m,0.0) for m in cal_ho]; sB150_ho=[moB150.get(m,0.0) for m in cal_ho]
    sS200_ho=[moS200.get(m,0.0) for m in cal_ho]; sS150_ho=[moS150.get(m,0.0) for m in cal_ho]
    sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_base_ho,_,_,_,_=bk([sB_base_ho,sS200_ho,sTB_ho],cal=cal_ho)
    sh_new_ho,_,_,_,_=bk([sB150_ho,sS150_ho,sTB_ho],cal=cal_ho)
    delta=sh_new_ho-sh_base_ho
    robust="✓ ROBUST" if delta>0 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: R74 Sh{sh_base_ho:+.2f} → R77 Sh{sh_new_ho:+.2f} delta{delta:>+.3f} {robust}")

# D: 7y BTC EMA150 vs 200 (structural?)
print(f"\n{'━'*100}"); print("D: 7y BTC EMA150 vs 200 structural verify")
moB7_200,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=200)
moB7_150,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},ema_1h=150)
sB7_200=[moB7_200.get(m,0.0) for m in cal7]; sB7_150=[moB7_150.get(m,0.0) for m in cal7]
sh_200_7=sharpe(sB7_200); sh_150_7=sharpe(sB7_150)
md_200_7=maxdd(sB7_200); md_150_7=maxdd(sB7_150)
print(f"  BTC78 EMA200 7y: Sh{sh_200_7:+.3f} DD{md_200_7:.1f}%")
print(f"  BTC78 EMA150 7y: Sh{sh_150_7:+.3f} DD{md_150_7:.1f}%")
print(f"  Delta 7y: Sh{sh_150_7-sh_200_7:+.3f}")
if sh_150_7>sh_200_7+0.03:
    print(f"  ✅ STRUCTURAL: 7y confirms EMA150 improvement")
elif sh_150_7>sh_200_7:
    print(f"  ⚠️ MARGINAL: 7y slightly positive")
else:
    print(f"  ❌ Not confirmed on 7y — overfit risk")

# E: Final ceiling table
print(f"\n{'━'*100}"); print("E: FINAL CEILING TABLE R50-R77")
print(f"  {'Config':<58} {'Sh':>7} {'DD':>5} {'delta':>8}")
print("  "+"-"*82)
final_configs=[
    ("R50 canonical",run_h01(f"{CC}/binance-5m-7y.json",18)[0],run_h01(f"{CC}/binance-sol-5m-3y.json",15)[0],run_turtle(15),[1,1,1]),
    ("R70 BTC78+SOL29+DX14+T1.2",None,None,None,[1,1,1.2]),
    ("R74 BTC78+SOL29+DX14+T1.2+Aug-skip",moB200,moS200,turB14,[1,1,1.2]),
    ("R76 +EMA150 BTC",moB150,moS200,turB14,[1,1,1.2]),
    ("R77 +EMA150 BTC+SOL [BEST?]",moB150,moS150,turB14,[1,1,1.2]),
]
from functools import reduce
for lbl,mB_,mS_,turB_,w in final_configs:
    if mB_ is None:
        mB_2,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28)
        mS_2,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24)
        mB_=mB_2; mS_=mS_2; turB_=turB14
    ...
    delta=sh-sh_base
    ok="✅✅✅" if sh>=2.30 else ("✅✅" if sh>=2.25 else ("✅" if sh>=2.10 else "⚠️"))
    print(f"  {ok} {lbl:<58} Sh{sh:>+6.3f} DD{md:>4.1f}% delta{delta:>+6.3f}")

print(f"\n{'='*100}")
print("R77 SUMMARY:")
print(f"  R76 EMA150 BTC: MOSTLY ROBUST (2/3 holdout, 7y = check D)")
print(f"  R77 SOL EMA150: Need B/C holdout verdict")
print(f"  IF BOTH robust → FINAL BEST: BTC150+SOL150+DX14+T1.2+Aug → Sh~+2.33")
print(f"  FINAL CONFIG R77 (if robust):")
print(f"    BTC: ADX18, SL2.78/3.28/16, EMA150-1h, RANGE-only, skip August")
print(f"    SOL: ADX15, SL2.9/3.4/24, EMA150-1h, RANGE-only")

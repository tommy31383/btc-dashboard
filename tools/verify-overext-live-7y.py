...
def main():
    ...
    # === Daily MA50 map cho overext gate (D-1 closed, no lookahead) ===
    dcs = [b["close"] for b in bars1d]
    day_ma50 = [None] * len(bars1d)
    for di in range(49, len(bars1d)):
        day_ma50[di] = sum(dcs[di - 49:di + 1]) / 50
    d_idx = {b["time"] // 86400000: i for i, b in enumerate(bars1d)}

    def overext_skip(ts, side, overext):
        # skip LONG nếu 1d-close (D-1 đã đóng) > MA50_1d × (1+overext) = blow-off top / vào trễ
        if overext <= 0 or side != "LONG":
            return False
        di = d_idx.get(ts // 86400000 - 1)
        if di is None or di < 49 or day_ma50[di] is None:
            return False
        return dcs[di] > day_ma50[di] * (1 + overext)
    ...
    def run(allowed_regimes, overext=0.0):
        trades = []
        ...
        for i in range(250, n - MAX_HOLD):
            for sn in ["S12", "S13", "S14"]:
                ...
                if overext_skip(bars4h[i]["time"], sig, overext): continue
                r = sim(i, sig)
    ...
    print("\nRunning OVEREXT sweep on LIVE config (Config A = RANGE_ONLY)...\n")
    OVS = [0.0, 0.10, 0.15, 0.20, 0.25]
    runs = {ov: run({"RANGE"}, ov) for ov in OVS}

    def metrics(trades):
        rets = [t["ret"] for t in trades]
        n_ = len(rets)
        if n_ == 0: return (0, 0.0, 0.0, 0.0, 0, 0, defaultdict(float))
        mean = sum(rets) / n_
        sd = (sum((r - mean) ** 2 for r in rets) / n_) ** 0.5 or 1e-9
        ra = mean / sd
        wr = sum(1 for r in rets if r > 0) / n_ * 100
        ...
        pos = sum(1 for v in by_yr.values() if v > 0)
        roi = sum(rets) * 100
        return (n_, roi, ra, wr, pos, len(by_yr), by_yr)

    print("=" * 78)
    print("OVEREXT GATE on LIVE hedge01 (RANGE-only v0.4.56) — return-% equal-weight, 7y")
    print("=" * 78)
    print(f"{'overext':>8} | {'n':>4} | {'ROI%':>8} | {'RA':>7} | {'WR%':>4} | {'stab':>5} | {'2021%':>8} | note")
    print("-" * 78)
    M = {}
    for ov in OVS:
        M[ov] = metrics(runs[ov])
        n_, roi, ra, wr, pos, ny, by_yr = M[ov]
        note = "LIVE baseline" if ov == 0 else ("prior 'win' 0.15" if ov == 0.15 else "")
        y21 = by_yr.get(2021, 0.0) * 100
        print(f"{ov:>8.2f} | {n_:>4} | {roi:>+8.1f} | {ra:>+7.3f} | {wr:>4.0f} | {pos:>2}/{ny:<2} | {y21:>+8.1f} | {note}")

    print("\nPer-year ROI% — baseline(0.00) vs overext(0.15):")
    by0 = M[0.0][6]; by15 = M[0.15][6]
    yrs = sorted(set(by0) | set(by15))
    print(f"  {'year':>6} | {'base':>9} | {'ov0.15':>9} | {'delta':>9}")
    for y in yrs:
        b = by0.get(y, 0) * 100; o = by15.get(y, 0) * 100
        print(f"  {y:>6} | {b:>+9.1f} | {o:>+9.1f} | {o - b:>+9.1f}")

    # === DOLLAR-WEIGHTED check (qty_base × entry_price × ret) ===
    QTY = {"S12": 0.005, "S13": 0.003, "S14": 0.005}  # live base qty per setup

    def dollars(trades):
        tot = 0.0; by = defaultdict(float)
        for t in trades:
            d = QTY[t["setup"]] * t["px"] * t["ret"]
            tot += d; by[t["yr"]] += d
        return tot, by

    print("\n" + "=" * 78)
    print("DOLLAR-WEIGHTED ($ = qty_base × entry_price × ret) — BTC price-scaled, 7y")
    print("=" * 78)
    print(f"{'overext':>8} | {'n':>4} | {'$ total':>9} | {'vs base':>8} | {'$2019':>7} | {'$2021':>7} | {'$2024':>7}")
    print("-" * 78)
    dbase = dollars(runs[0.0])[0]
    for ov in OVS:
        tot, by = dollars(runs[ov])
        dv = "" if ov == 0.0 else f"{tot - dbase:+.0f}"
        print(f"{ov:>8.2f} | {len(runs[ov]):>4} | {tot:>+9.0f} | {dv:>8} | {by.get(2019,0):>+7.0f} | {by.get(2021,0):>+7.0f} | {by.get(2024,0):>+7.0f}")

    print("\n--- Detail baseline (overext=0.00) ---")
    report(runs[0.0], "LIVE baseline RANGE-only")
    print("\n--- Detail overext=0.15 ---")
    report(runs[0.15], "RANGE-only + overext 0.15")

    print("\n" + "=" * 78)
    print("VERDICT GUIDE:")
    print("  overext ~ no change vs baseline  → RANGE-only đã loại overextended → KHÔNG port (no-op).")
    print("  overext cải thiện ROI+2021 không hại bull → edge thật trên live → port.")
    print("=" * 78)

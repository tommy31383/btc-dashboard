/**
 * mark-clean-5pct-5m.ts (anh Tommy 2026-05-04)
 * Đánh dấu cây 5m mà SAU đó:
 *   - Giá tăng ≥ 5% (high tương lai >= entry × 1.05)
 *   - VÀ trước khi đạt 5%, giá KHÔNG giảm quá 2% (MAE < 2%)
 * Lookforward: 7 ngày = 2016 bars 5m.
 * Sau đó analyze features so với baseline.
 */
...
const LOOKFORWARD_BARS = 2016; // 7d × 24h × 12 bars/h
...
function main() {
  console.log("[mark5pct] Loading 5m...");
  ...
  console.log(`[mark5pct] ${c.length} bars`);
  ...
  console.log(`\n=== RESULTS (TP+${TARGET_PCT}%, MAE<${MAX_ADVERSE_PCT}%, 7d window) ===`);
  console.log(`Total: ${c.length-LOOKFORWARD_BARS-20}`);
  console.log(`✅ CLEAN winners: ${winners.length} (${(winners.length/(c.length-LOOKFORWARD_BARS-20)*100).toFixed(2)}%)`);
  console.log(`❌ Dirty (SL hit): ${dirtySL} (${(dirtySL/(c.length-LOOKFORWARD_BARS-20)*100).toFixed(2)}%)`);
  ...
  const winIdx = winSample.map(w=>{
    let lo=0, hi=c.length-1, ans=-1;
    while (lo<=hi){const m=(lo+hi)>>1; if (c[m].time<=w.ts){ans=m; lo=m+1;} else hi=m-1;}
    return ans;
  }).filter(i=>i>=20);
  ...
  console.log(`\n[features] Computing ${winIdx.length} winners + ${baseIdx.length} baseline...`);
  ...
  console.log(`\n=== BINARY FILTER TEST (base ${baseRate.toFixed(1)}%) ===`);
  const conds: { name: string; pred: (idx: number)=>boolean }[] = [
    ...
    {name:"COMBO: RSI<30 + dnWick≥0.5", pred:i=>(rsi[i]??50)<30 && (Math.min(c[i].open,c[i].close)-c[i].low)/c[i].open*100>=0.5},
  ...
  for (const cond of conds) {
    ...
    console.log(`  ${cond.name.padEnd(34)}: ${total.toString().padStart(7)} bars → ${win.toString().padStart(6)} clean (${wr.toFixed(1)}%, lift ${lift.toFixed(2)}×) ${lift>=1.3?"⭐":""}`);
  }
  ...
  writeFileSync(join(__dirname,"..","assets","mark_clean_5pct_5m.json"), JSON.stringify({
    config:{tf:"5m", targetPct:TARGET_PCT, maxAdversePct:MAX_ADVERSE_PCT, lookforwardBars:LOOKFORWARD_BARS, lookforwardDays:LOOKFORWARD_BARS*5/60/24},
    period:{start:c[0].time, end:c[c.length-1].time},
    totalBars: c.length, winnerCount: winners.length, dirtySL, neverTP,
    discriminating: scored,
    winners: winners.length>10000 ? winners.filter((_,i)=>i%Math.ceil(winners.length/10000)===0) : winners,
    priceLine,
  }));
  console.log(`\nSaved → assets/mark_clean_5pct_5m.json`);
}
main();

#!/usr/bin/env python3
"""
backtest-atr-pct-sweep-7y.py — ATR percentile sweep vs baseline (PA2: relax ATR 50th)

So sánh ATR_PCT_PCTL: 0.35 / 0.40 / 0.45 / 0.50 (current baseline)
Config đầy đủ v0.4.56:
  SL_INIT=4, SL_TRAIL=3, SL_TRANS=24 bars (96h)
  RANGE_ONLY=True, SKIP_SHORT=True
  SKIP_H16=True, SKIP_THU_SUN=True
  ADX_THRESHOLD=20, ADX_STICKY=True (Wilder)
  VOL_MA=10, VOL_MULT=1.2
  ATR_BREAK_MULT=1.2

Objective: tìm ATR percentile thấp hơn 50th vẫn accept 4-metric:
  RA >= baseline, Stab >= baseline, WR/R:R không tệ hơn nhiều
"""
...
def main():
    ...
    def filt(i, side, pct):
        ...
        if not atp_pass(i, pct): return False
    ...
    def run(pct):
        ...
        for i in range(250, n - MAX_HOLD):
            for sn in ["S12", "S13", "S14"]:
                ...
                if not filt(i, sig, pct): continue
                r = sim(i, sig)
    ...
    print("\nRunning ATR percentile sweep (BTC 7y)...\n")
    results = []
    for pct in SWEEP:
        trades = run(pct)
        label = f"ATR_PCT={int(pct*100)}th {'<-- BASELINE' if pct == 0.50 else ''}"
        r = report(trades, label)
        if r:
            results.append((pct, r[0], r[1]))

    print("\n" + "=" * 60)
    print("SUMMARY TABLE")
    print(f"  {'pct':>8}  {'RA':>8}  {'n':>6}  {'Δn vs 50th':>12}  {'ΔRA vs 50th':>12}")
    base_ra = base_n = None
    for pct, ra, n_ in results:
        if pct == 0.50:
            base_ra = ra; base_n = n_
    for pct, ra, n_ in results:
        dn = n_ - base_n if base_n else 0
        dra = ra - base_ra if base_ra else 0
        flag = " <-- BASELINE" if pct == 0.50 else ""
        print(f"  {int(pct*100):>7}th  {ra:>+8.3f}  {n_:>6}  {dn:>+12}  {dra:>+12.3f}{flag}")

    print("\nAccept criteria: RA >= baseline - 0.02 AND stab không tệ hơn")

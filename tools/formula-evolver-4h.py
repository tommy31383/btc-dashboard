...
CHAMP=os.path.join(HERE,"formula4h-champions.json");STOP=os.path.join(HERE,"formula4h-STOP")
...
def rand_genome():
    return {"dir":random.choice(["LONG","SHORT"]),
            ...
            "sl":round(random.uniform(1.2,3.0),1),"trail":round(random.uniform(1.8,4.0),1),"maxhold":random.choice([6,12,24,48])}
def mut(g):
...
def gates(g,AD,track=None):
    Db=AD["BTC"];tr=test(g,*Db)
    if len(tr)<150:return None
    by=defaultdict(list)
    for t in tr:by[yr(t["t"])].append(t)
    rets=[x["ret"] for x in tr]
    if sum(rets)<=0:return None
    srt=sorted(rets,reverse=True);cut=max(5,len(rets)//5)
    if sum(srt[cut:])<=0:return None
    if sum(x["ret"] for x in by.get(2026,[]))<0:return None
    posY=sum(1 for y in by if len(by[y])>=5 and sum(x["ret"] for x in by[y])>0)
    totY=sum(1 for y in by if len(by[y])>=5)
    if totY<5 or posY/totY<0.6:return None
    rsum=sum(rets);wins=0
    for s2 in range(20):
        rng=random.Random(s2*97+len(tr));picks=set(rng.sample(range(30,len(Db[1])-1),min(len(tr),len(Db[1])-35)))
        if rsum>sum(x["ret"] for x in _rand_test({"dir":g["dir"],"sl":g["sl"],"trail":g["trail"],"maxhold":g["maxhold"]},Db,picks)):wins+=1
    if track is not None and wins>track[0]:track[0]=wins;track[1]=dict(n=len(tr),sumRet=round(rsum*100),wins=wins,stab=f"{posY}/{totY}")
    if wins<18:return None                              # BTC beat random 18/20 (pct90)
    return {"genome":g,"n":len(tr),"sumRet":round(rsum*100,1),"dropTop20":round(sum(srt[cut:])*100,1),
            "ret26":round(sum(x["ret"] for x in by.get(2026,[]))*100,1),"n26":len(by.get(2026,[])),
            "randWin":f"{wins}/20","stab":f"{posY}/{totY}",
            "wr":round(sum(1 for x in rets if x>0)/len(rets)*100,1),"ts":int(time.time())}
    ...
    while not os.path.exists(STOP):
        ...
        if res:
            ...
            if ok:
                ...
                c=res
                print(f"[formula4h] it{it} ✓#{found} {c['genome']['dir']} n{c['n']} medA{c['medA']:+.2f}% SOL{c['medSOL']:+.2f} ETH{c['medETH']:+.2f} 26:{c['ret26']:+.0f} 22:{c['ret22']:+.0f} WR{c['wr']}%")
        if it%200000==0:print(f"[formula4h] it{it} found={found} | best near-miss random {best[0]}/20 {best[1]}")
    print(f"[formula4h] STOP {it} iters {found} found")

#!/usr/bin/env python3
"""autoloop-r85-sol-abm.py — Round 85: SOL ABM=1.5 verify
R84: SOL ABM=1.5 → Sh+2.43 DD4.6% (+0.054 ★★★★, n=34 vs 39)
  A: SOL ABM fine sweep 1.3-1.8 — confirm peak
  B: SOL ABM=1.5 robustness — holdout years
  C: SOL ABM=1.5 + SL micro
  D: Combine SOL ABM=1.5 with all other improvements
  E: BTC ABM sweep with R83 config (was 1.3 optimal)
"""
...
moS_13,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=1.3)
moS_15,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=1.5)
turB14=run_turtle()
H.CACHE=f"{CC}/binance-sol-5m-3y.json"; bs=H.load_tf(H.H4); span_s=(bs[0]["time"],bs[-1]["time"])
H.CACHE=f"{CC}/binance-5m-7y.json"
cal3=months_between(span_s[0],span_s[1])
sB=[moB.get(m,0.0) for m in cal3]; sS13=[moS_13.get(m,0.0) for m in cal3]
sS15=[moS_15.get(m,0.0) for m in cal3]; sTB14=[turB14.get(m,0.0) for m in cal3]; W=[1,1,1.2]
...
def pr(label, parts, weights=None, cal=None, extra=""):
    ...
    ok=sh>=2.40; mark="✅✅✅✅✅" if ok else ("✅✅✅✅" if sh>=2.35 else ("✅✅✅" if sh>=2.30 else ("✅✅" if sh>=2.25 else ("✅" if sh>=2.10 else ("⚠️" if sh>=2.00 else "")))))
    star="★★★★★" if sh>=2.50 else ("★★★★" if sh>=2.45 else ("★★★" if sh>=2.40 else ("★★" if sh>=2.38 else ("★" if delta>0.05 else ""))))
    print(f"  {mark} {label:<54} Sh{sh:>+5.2f} DD{md:>4.1f}% flat{fl:>3} TE{te:>+5.2f} d{delta:>+.3f} {star} {extra}")
    return sh,md

sh_base,_=pr("R83 BASELINE (SOL ABM=1.3)",[sB,sS13,sTB14])
pr("  SOL ABM=1.5 (R84 find)",[sB,sS15,sTB14])
print(); print("="*100); print("=== R85: SOL ABM=1.5 verify ===\n")

# A: SOL ABM fine sweep
print("━"*100); print("A: SOL ABM fine sweep 1.3-1.8")
best_abm=1.3; best_abm_sh=sh_base; best_moS=moS_13
for abm_v in [1.30,1.35,1.40,1.45,1.50,1.55,1.60,1.70,1.80]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=abm_v)
    sS_=[moS_.get(m,0.0) for m in cal3]
    sh,md=pr(f"  SOL ABM={abm_v}",[sB,sS_,sTB14],extra=f"n={nt}")
    if sh>best_abm_sh: best_abm_sh=sh; best_abm=abm_v; best_moS=moS_
print(f"\n  → Best SOL ABM={best_abm} Sh{best_abm_sh:+.3f}")

# B: SOL ABM=1.5 robustness
print(f"\n{'━'*100}"); print("B: SOL ABM=1.5 vs ABM=1.3 robustness — holdout years")
for yr_exc in [2023,2024,2025]:
    cal_ho=[m for m in cal3 if not m.startswith(str(yr_exc))]
    sB_ho=[moB.get(m,0.0) for m in cal_ho]
    sS13_ho=[moS_13.get(m,0.0) for m in cal_ho]; sS15_ho=[moS_15.get(m,0.0) for m in cal_ho]
    sTB_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_13,_,_,_,_=bk([sB_ho,sS13_ho,sTB_ho],cal=cal_ho)
    sh_15,md_15,_,_,_=bk([sB_ho,sS15_ho,sTB_ho],cal=cal_ho)
    delta=sh_15-sh_13
    robust="✓ ROBUST" if delta>0 else "✗ FRAGILE"
    print(f"  Holdout {yr_exc}: ABM1.3 Sh{sh_13:+.2f} → ABM1.5 Sh{sh_15:+.2f} DD{md_15:.1f}% delta{delta:>+.3f} {robust}")

# C: SOL ABM=best + SL micro
print(f"\n{'━'*100}"); print(f"C: SOL ABM={best_abm} + SL sweep")
sBest_moS=[best_moS.get(m,0.0) for m in cal3]
for si,st in [(2.85,3.35),(2.88,3.38),(2.90,3.40),(2.92,3.42)]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=si,sl_trail=st,sl_trans=24,donchian_lb=20,vol_ma=16,atr_break_mult=best_abm)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL SL{si}/{st} ABM{best_abm}",[sB,sS_,sTB14],extra=f"n={nt}")

# D: Full combine
print(f"\n{'━'*100}"); print(f"D: Full combine — all R85 improvements")
pr(f"  R83 BASELINE (ABM1.3)",[sB,sS13,sTB14])
pr(f"  + SOL ABM={best_abm}",[sB,sBest_moS,sTB14])

# E: BTC ABM sweep with full R83+ config
print(f"\n{'━'*100}"); print("E: BTC ABM sweep with SOL ABM=1.5")
for abm_v in [1.2,1.3,1.4,1.5]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16,atr_break_mult=abm_v)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC ABM={abm_v}",[sB_,sS15,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R85 SUMMARY — ceiling Sh{sh_base:+.3f}")
print(f"  Best SOL ABM: {best_abm} → Sh{best_abm_sh:+.3f}")
print(f"  SOL ABM=1.5 ROBUST? → see section B")

#!/usr/bin/env python3
"""autoloop-r88-explore-r87.py — Round 88: Explore with R87 config
R87 FINAL: SOL ST=20 → Sh+2.48 DD4.6% (2/3 robust, 2024 slight -0.019)

R88:
  A: Full verify R87 vs R50
  B: BTC SL_TRANS with R87 config (was 16, C found 12-16 all same) — re-check
  C: NEW: Turtle weight tune with R87 config (DD4.6%, maybe T_w can be different)
  D: NEW: SOL ADX_PERIOD with ABM=1.5+ST=20 (might interact)
  E: NEW: BTC ADX_PERIOD with R87 full config
"""
...
sh_base,_=pr("R87 FINAL",[sB,sS,sTB14])
print(); print("="*100); print("=== R88: Explore with R87 config ===\n")

# A: Full verify R87 vs R50
print("━"*100); print("A: R87 vs R50 — full holdout")
...
def run_turtle15():
    ...
    for i in range(1,nd):
        h=BD[i]["high"]; l=BD[i]["low"]; pc=BD[i-1]["close"]
        atrd[i]=max(h-l,abs(h-pc),abs(l-pc))
    dhi=[None]*nd; dlo=[None]*nd; DE=20; DX=15
    for i in range(DE,nd): dhi[i]=max(BD[j]["high"] for j in range(i-DE,i))
    for i in range(DX,nd): dlo[i]=min(BD[j]["low"] for j in range(i-DX,i))
    tur_mo=defaultdict(float); hold=False; e=0.0; a=0.0
    for i in range(max(DE,DX),nd):
...
for yr_exc in [2023,2024,2025]:
    ...
    sBcan_ho=[moB_can.get(m,0.0) for m in cal_ho]; sB_ho=[moB.get(m,0.0) for m in cal_ho]
    sScan_ho=[moS_can.get(m,0.0) for m in cal_ho]; sS_ho=[moS.get(m,0.0) for m in cal_ho]
    sTB15_ho=[turB15.get(m,0.0) for m in cal_ho]; sTB14_ho=[turB14.get(m,0.0) for m in cal_ho]
    sh_can,md_can,_,_,_=bk([sBcan_ho,sScan_ho,sTB15_ho],[1,1,1],cal=cal_ho)
    sh_r87,md_r87,_,_,_=bk([sB_ho,sS_ho,sTB14_ho],W,cal=cal_ho)
    delta=sh_r87-sh_can
    print(f"  Holdout {yr_exc}: R50 Sh{sh_can:+.2f} DD{md_can:.1f}% → R87 Sh{sh_r87:+.2f} DD{md_r87:.1f}% delta{delta:>+.3f} {'✓' if delta>0 else '✗'}")

# B: Turtle weight with R87
print(f"\n{'━'*100}"); print("B: Turtle weight tune with R87 config")
for wt in [0.8,0.9,1.0,1.1,1.2,1.3,1.4,1.5]:
    sh,md=pr(f"  T_w={wt}",[sB,sS,sTB14],[1,1,wt])

# C: SOL ADX_P with R87
print(f"\n{'━'*100}"); print("C: SOL ADX_P with R87 config (current=12)")
for ap in [8,10,12,14,16]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=20,donchian_lb=20,vol_ma=16,atr_break_mult=1.5,adx_period=ap)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL ADX_P={ap}",[sB,sS_,sTB14],extra=f"n={nt}")

# D: BTC ADX_P with R87
print(f"\n{'━'*100}"); print("D: BTC ADX_P with R87 config (current=12)")
for ap in [10,11,12,13,14,16]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},donchian_lb=18,vol_ma=16,adx_period=ap)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  BTC ADX_P={ap}",[sB_,sS,sTB14],extra=f"n={nt}")

# E: SOL ABM fine-tune with ST=20 (might have different optimal)
print(f"\n{'━'*100}"); print("E: SOL ABM sweep with ST=20 (re-confirm 1.5)")
for abm in [1.3,1.4,1.45,1.5,1.55,1.6,1.7]:
    moS_,nt=run_h01(f"{CC}/binance-sol-5m-3y.json",15,sl_init=2.9,sl_trail=3.4,sl_trans=20,donchian_lb=20,vol_ma=16,atr_break_mult=abm)
    sS_=[moS_.get(m,0.0) for m in cal3]
    pr(f"  SOL ABM={abm}",[sB,sS_,sTB14],extra=f"n={nt}")

print(f"\n{'='*100}")
print(f"R88 SUMMARY — Ceiling Sh{sh_base:+.3f}")
print(f"  R87 CONFIRMED FINAL CONFIG: Sh+2.48 DD4.6%")
print(f"    BTC: ADX18, SL2.78/3.28/16, D_LB=18, vol_ma=16, ABM=1.3, skip Aug")
print(f"    SOL: ADX15, SL2.9/3.4/20, D_LB=20, vol_ma=16, ABM=1.5")

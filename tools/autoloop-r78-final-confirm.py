#!/usr/bin/env python3
"""autoloop-r78-final-confirm.py — Round 78: Final confirmation + last directions
R77 CRITICAL: EMA150 7y Sh-0.045 (vs 3y +0.054) → OVERFIT! REJECTED.
R74 config remains best: BTC78+SOL29+DX14+T1.2+Aug-skip → Sh+2.26 DD5.3%

R78:
  A: Final config 7y verify — R74 config on 7y BTC standalone
  B: NEW DIRECTION — Turtle exit channel on SOL (different DX for SOL turtle)... wait SOL has no turtle
  B: NEW DIRECTION — BTC DONCHIAN_LB (S14 lookback) fine-sweep với Aug-skip config
  C: NEW DIRECTION — vol_mult with Aug-skip (might interact)
  D: NEW DIRECTION — SL_TRANS (init→trail switchover) with Aug-skip
  E: FINAL DEFINITIVE CEILING TABLE
"""
...
print("Loading R74 CONFIRMED final config...")
moB_f,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8})
...
sB_f=[moB_f.get(m,0.0) for m in cal3]; sS_f=[moS_f.get(m,0.0) for m in cal3]
...
sh_base,_=pr("R74 FINAL CONFIG",[sB_f,sS_f,sTB14])
print(); print("="*100); print("=== R78: Final confirm + last directions ===\n")

# A: 7y BTC verify R74 config
print("━"*100); print("A: 7y BTC verify — R74 config standalone")
moB7_can,_=run_h01(f"{CC}/binance-5m-7y.json",18)
moB7_new,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28)
moB7_aug,_=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8})
sB7c=[moB7_can.get(m,0.0) for m in cal7]; sB7n=[moB7_new.get(m,0.0) for m in cal7]
sB7a=[moB7_aug.get(m,0.0) for m in cal7]
print(f"  BTC canonical 7y: Sh{sharpe(sB7c):+.2f} DD{maxdd(sB7c):.0f}%")
print(f"  BTC SL2.78 7y: Sh{sharpe(sB7n):+.2f} DD{maxdd(sB7n):.0f}% delta{sharpe(sB7n)-sharpe(sB7c):+.3f}")
print(f"  BTC SL2.78+Aug-skip 7y: Sh{sharpe(sB7a):+.2f} DD{maxdd(sB7a):.0f}% delta{sharpe(sB7a)-sharpe(sB7c):+.3f}")

# B: DONCHIAN_LB sweep with Aug-skip
print(f"\n{'━'*100}"); print("B: DONCHIAN_LB (S14) sweep with Aug-skip (was 18 optimal without Aug?)")
for dlb in [15,16,17,18,19,20,22,25]:
...
# C: vol_mult with Aug-skip
print(f"\n{'━'*100}"); print("C: vol_mult sweep with Aug-skip")
for vm in [1.2,1.3,1.4,1.5,1.6]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},vol_mult=vm)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  vol_mult={vm}",[sB_,sS_f,sTB14],extra=f"n={nt}")

# D: SL_TRANS with Aug-skip
print(f"\n{'━'*100}"); print("D: SL_TRANS sweep with Aug-skip (current=16)")
for st_ in [8,10,12,14,16,20,24]:
    moB_,nt=run_h01(f"{CC}/binance-5m-7y.json",18,sl_init=2.78,sl_trail=3.28,skip_months={8},sl_trans=st_)
    sB_=[moB_.get(m,0.0) for m in cal3]
    pr(f"  SL_TRANS={st_}",[sB_,sS_f,sTB14],extra=f"n={nt}")

# E: FINAL CEILING TABLE
print(f"\n{'━'*100}"); print("E: DEFINITIVE FINAL CEILING TABLE R50-R78")
print(f"  All improvements proven 7y (not just 3y):")
print()
can_moB,_=run_h01(f"{CC}/binance-5m-7y.json",18)
can_moS,_=run_h01(f"{CC}/binance-sol-5m-3y.json",15)
turB15=run_turtle(15)
configs=[
    ("R50 CANONICAL",can_moB,can_moS,turB15,[1,1,1]),
    ("R70 SL tightening (BTC78+SOL29+DX14+T1.2)",None,None,None,W),
    ("R74 +Aug-skip [CONFIRMED FINAL]",moB_f,moS_f,turB14,W),
]
for lbl,mB_,mS_,turB_,w in configs:
    if mB_ is None:
...
print(f"\n{'='*100}")
print("R78 SUMMARY — CONFIRMED FINAL CEILING:")
print(f"  CONFIG: BTC(ADX18,SL2.78/3.28/16,RANGE,Aug-skip) + SOL(ADX15,SL2.9/3.4/24) + Turtle(DE20/DX14/CUT2.0/T1.2)")
print(f"  CEILING: Sh~+2.26 DD5.3% (3y calendar 2023-2025)")
print(f"  REJECTED: EMA150 (7y fail), Sep-skip (holdout fail), D_LB=18 (overfit)")
print(f"  ACCEPT-WITH-CAVEAT: Aug skip (7y +0.055, n=3 real trades)")
print(f"  → FORWARD-TEST REQUIRED before sizing up")

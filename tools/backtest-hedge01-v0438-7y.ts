/**
 * backtest-hedge01-v0438-7y.ts — Validate hedge01 v0.4.38 trên 7y full-cycle data.
 *
 * Stages:
 *   A. Full 7y backtest v0.4.38 (17 setups, 3 buckets)
 *   B. Per-setup audit top 5 isolated standalone
 *   C. Walk-forward: train 2019-22 (4y) → test 2023-26 (3y)
 *
 * Data: 5m 7y bars → derive 15m/1h/4h/1d/1w on-the-fly.
 * Funding rate: chỉ có 3y data → S15 skip ngoài range, OK acceptable.
 */
...
function loadFunding(): FundingPoint[] {
  try { return JSON.parse(readFileSync(join(__dirname, "..", ".cache", "binance-funding-3y.json"), "utf8")); }
  catch { return []; }
}

/** Aggregate 5m bars → target TF (e.g. 15m → group 3 bars). */
function aggregateBars(c5: Candle[], targetMinutes: number): Candle[] {
  const groupSize = targetMinutes / 5;
  if (groupSize < 1 || !Number.isInteger(groupSize)) throw new Error(`Invalid target ${targetMinutes}m`);
  const targetMs = targetMinutes * 60_000;
  const out: Candle[] = [];
  // Align bars by floor(time / targetMs)
...
interface ScenarioMask {
  ...
  enableTrend: boolean;   // S12/S13/S14
  enableS15: boolean;
  enableS16: boolean;
  enableS17: boolean;
  enableShort: boolean;
...
const ALL_ENABLED: ScenarioMask = {
  enableAggregate: true, enableSetup11: true, enableTrend: true,
  enableS15: true, enableS16: true, enableS17: true, enableShort: true,
};
...
function runBacktest(
  c5: Candle[], c15: Candle[], c1h: Candle[], c4h: Candle[], c1d: Candle[], c1w: Candle[],
  funding: FundingPoint[], ind: any, mask: ScenarioMask,
  ...
  // Pre-compute vol MA 20-bar 5m + BB 5m + RSI 5m (for S17)
  const volMA_5 = ind.volMA_5;
  const ma20_5 = ind.ma20_5;
  const sd20_5 = ind.sd20_5;
  const rsi5full = ind.rsi5;
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    let c2Fire = false;
    if (idx5c >= 22) {
      const last5 = c5[idx5c]; const r5 = rsi5full[idx5c];
      if (r5 !== null && r5 >= 70) {
    ...
    // Setup #15 (funding may not exist for early period)
    ...
    if (mask.enableS17 && i >= 21) {
      const r5_now = rsi5full[i]; const m20_now = ma20_5[i]; const s20_now = sd20_5[i];
...
function main() {
  ...
  console.log(`  15m=${c15.length} 1h=${c1h.length} 4h=${c4h.length} 1d=${c1d.length} 1w=${c1w.length}`);

  console.log("\n[7y] Pre-computing indicators...");
  ...
  // === STAGE A: Full 7y ===
  console.log("\n=== STAGE A: hedge01 v0.4.38 full 7y ===");
  const rA = runBacktest(c5, c15, c1h, c4h, c1d, c1w, funding, ind, ALL_ENABLED, 0, c5.length, "A_full_7y");
  console.log(`  ROI ${rA.roi}% / DD ${rA.maxDD}% / RA ${rA.ra} / ${rA.entries}E ${rA.closes}C WR ${rA.wr}% R:R ${rA.rr} Exp $${rA.exp}`);
  const yearsA = Object.keys(rA.byYear).sort();
  console.log("  Per-year:");
  for (const y of yearsA) {
    const yd = rA.byYear[y];
    const sign = yd.pnl >= 0 ? "+" : "";
    console.log(`    ${y}: ${yd.entries}E ${yd.closes}C ${sign}$${yd.pnl}`);
  }
  const yearsPos = yearsA.filter(y => rA.byYear[y].pnl > 0).length;
  console.log(`  Stability: ${yearsPos}/${yearsA.length} năm dương`);

  // === STAGE B: Per-setup audit (8 scenarios — each isolated) ===
  console.log("\n=== STAGE B: per-setup standalone (each isolated) ===");
  const setupMasks: { name: string; mask: ScenarioMask }[] = [
    { name: "agg_only", mask: { ...ALL_ENABLED, enableSetup11: false, enableTrend: false, enableS15: false, enableS16: false, enableS17: false } },
    { name: "S11_only", mask: { enableAggregate: false, enableSetup11: true, enableTrend: false, enableS15: false, enableS16: false, enableS17: false, enableShort: false } },
    { name: "trend_S12_S13_S14_only", mask: { enableAggregate: false, enableSetup11: false, enableTrend: true, enableS15: false, enableS16: false, enableS17: false, enableShort: false } },
    { name: "S15_only", mask: { enableAggregate: false, enableSetup11: false, enableTrend: false, enableS15: true, enableS16: false, enableS17: false, enableShort: false } },
    { name: "S16_only", mask: { enableAggregate: false, enableSetup11: false, enableTrend: false, enableS15: false, enableS16: true, enableS17: false, enableShort: false } },
    { name: "S17_only", mask: { enableAggregate: false, enableSetup11: false, enableTrend: false, enableS15: false, enableS16: false, enableS17: true, enableShort: false } },
    { name: "no_short_aggregate", mask: { ...ALL_ENABLED, enableShort: false } },
  ];
  const stageB: any[] = [];
  for (const { name, mask } of setupMasks) {
    console.log(`  Running ${name}...`);
    const r = runBacktest(c5, c15, c1h, c4h, c1d, c1w, funding, ind, mask, 0, c5.length, name);
    stageB.push(r);
  }

  // === STAGE C: Walk-forward 2019-22 train / 2023-26 test ===
  ...
  console.log(`  Split idx ${splitIdx} (${new Date(c5[splitIdx].time).toISOString()})`);
  const rTrain = runBacktest(c5, c15, c1h, c4h, c1d, c1w, funding, ind, ALL_ENABLED, 0, splitIdx, "C_train_2019_22");
  const rTest = runBacktest(c5, c15, c1h, c4h, c1d, c1w, funding, ind, ALL_ENABLED, splitIdx, c5.length, "C_test_2023_26");

  console.log("\n=== FINAL REPORT ===");
  console.log("\n--- Stage A: Full 7y ---");
  console.log(`ROI ${rA.roi}% / DD ${rA.maxDD}% / RA ${rA.ra} / ${rA.entries}E ${rA.closes}C WR ${rA.wr}% R:R ${rA.rr} | Stability ${yearsPos}/${yearsA.length}`);

  console.log("\n--- Stage B: per-setup contribution ---");
  console.log("Scenario                      | ROI%   | DD%   | RA    | Entries | WR%   | R:R  | Exp/trade");
  ...
  console.log("-".repeat(110));
  for (const r of [rTrain, rTest]) {
    console.log(`${r.scenario.padEnd(29)} | ${String(r.roi).padStart(6)} | ${String(r.maxDD).padStart(5)} | ${String(r.ra).padStart(5)} | ${String(r.entries).padStart(7)} | ${String(r.wr).padStart(5)} | ${String(r.rr).padStart(4)} | ${String(r.exp).padStart(8)}`);
  }

  console.log("\n--- Stage A: per-year detail ---");
  for (const y of yearsA) {
    const yd = rA.byYear[y];
    const sign = yd.pnl >= 0 ? "+" : "";
  ...
  const output = {
    stageA_full_7y: rA,
    stageB_per_setup: stageB,
    stageC_walkforward: { train: rTrain, test: rTest },
  };

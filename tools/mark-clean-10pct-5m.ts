/**
 * mark-clean-10pct-5m.ts (anh Tommy 2026-05-04)
 * Mark cây 5m mà sau đó giá tăng ≥10% với MAE<2%, lookforward 14d.
 * Phân tích features predictive + binary filter lift.
 */
...
const LOOKFORWARD_BARS = 4032; // 14 days × 24h × 12 bars/h
...
function main() {
  console.log(`[mark-10pct-5m] Loading 5m... TP+${TARGET_PCT}% / MAE<${MAX_ADVERSE_PCT}% / lookforward ${LOOKFORWARD_BARS*5/60/24}d`);
  ...
  console.log(`[mark-10pct-5m] ${c.length} bars`);
  ...
  console.log(`✅ CLEAN winners: ${winners.length} (${(winners.length/total*100).toFixed(2)}%)`);
  console.log(`❌ Dirty (SL hit): ${dirtySL} (${(dirtySL/total*100).toFixed(2)}%)`);
  console.log(`❌ Never TP: ${neverTP} (${(neverTP/total*100).toFixed(2)}%)`);
  ...
  const conds: { name: string; pred: (idx: number)=>boolean }[] = [
    ...
    {name:"COMBO RSI<30+distMA200<-10", pred:i=>{const m=ma200[i]; if(!m) return false; return (rsi[i]??50)<30 && (c[i].close-m)/m*100<-10;}},
  ];
  for (const cond of conds) {
    ...
    console.log(`  ${cond.name.padEnd(34)}: ${totalMatch.toString().padStart(7)} bars → ${win.toString().padStart(6)} clean (${wr.toFixed(1)}%, lift ${lift.toFixed(2)}×) ${lift>=1.5?"⭐⭐":lift>=1.3?"⭐":""}`);
  }
  ...
  writeFileSync(join(__dirname,"..","assets","mark_clean_10pct_5m.json"), JSON.stringify({
    ...
    totalBars:c.length, winnerCount:winners.length, dirtySL, neverTP, winnerPct:winners.length/total*100,
    discriminating:scored,
    winners: winners.length>10000 ? winners.filter((_,i)=>i%Math.ceil(winners.length/10000)===0) : winners,
    priceLine,
  }));
  console.log(`\nSaved → assets/mark_clean_10pct_5m.json`);
}
main();
